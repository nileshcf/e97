...
/**
 * The S918test application can be started in three different ways
 * 1. Via mainframe JCL
	...
	public static void main(String[] args) {
		...
		if (StringUtils.hasText(appType) && WebApplicationType.NONE.equals(WebApplicationType.valueOf(appType.trim().toUpperCase())))
            ...
            GlobalExecutorCtx.load(CFApplication.class, context);
		 		...
		 		S918test s918test = (S918test) context.getBean("s918test");
        	    s918test.process(new GlobalExecutorCtx().getContext(S918testCtx.class)); /*  S918test requires no input parameters  */
