...
public class MonPriKeySerialized  extends Field { 
   ...
   /*  offset of each of Child Fields when serialized as a String */
            protected  int beginMonXcdtTskStrtDt;
	...
	**/
	...
	protected void init(int begin) {
	   ...
	   /*  set the offset/position of each field when this object is serialized as String */
             beginMonXcdtTskStrtDt = getStartOffset() + 0;	// set offset for serialization
     ...
     int localMonXcdtTskStrtDtCounter = -1;
     public boolean isMonXcdtTskStrtDtModified() {
         int sharedCounter = shareString.getSerializedField().getModifiedCounter(); 
         boolean hasModified = localMonXcdtTskStrtDtCounter != sharedCounter;
         localMonXcdtTskStrtDtCounter = sharedCounter; return hasModified;
     }
	protected static final int MON_XCDT_TSK_STRT_DT_LEN = 10;
	/**
	 * 	serialize this MonXcdtTskStrtDt
	 */
   protected void serializeMonXcdtTskStrtDt(char[] monXcdtTskStrtDt) {
        shareString.getSerializedField().incrementCounter();
        arraycopy(monXcdtTskStrtDt,0,getStringValue(),beginMonXcdtTskStrtDt,MON_XCDT_TSK_STRT_DT_LEN);
       localMonXcdtTskStrtDtCounter = shareString.getSerializedField().getModifiedCounter();  	
   }

   protected char[] checkMonXcdtTskStrtDtConstraints(char[] value) {
   			return super.checkConstraints(value , 10 ,false, false);
   }
    /**
	 *	refreshMonXcdtTskStrtDt is used to refresh the latest value of a variable from the Serialized String
	 *  the most common reason to serialize an Object as a string in to write to a file. There can be several other reasons for serialization as well
	 */ 
   	public char[] refreshMonXcdtTskStrtDt() {	 
   		return (substring(getStringValue(),beginMonXcdtTskStrtDt,beginMonXcdtTskStrtDt + MON_XCDT_TSK_STRT_DT_LEN));
   	}
