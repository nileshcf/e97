...
/**
 * The Callip application can be started in three different ways
 * 1. Via mainframe JCL
	...
	public static void main(String[] args) {
		...
		if (StringUtils.hasText(appType) && WebApplicationType.NONE.equals(WebApplicationType.valueOf(appType.trim().toUpperCase())))
            ...
            GlobalExecutorCtx.load(CFApplication.class, context);
		 		...
		 		Callip callip = (Callip) context.getBean("callip");
        	    callip.process(new GlobalExecutorCtx().getContext(CallipCtx.class)); /*  Callip requires no input parameters  */
