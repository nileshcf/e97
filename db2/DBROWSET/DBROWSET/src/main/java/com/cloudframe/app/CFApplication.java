...
/**
 * The Dbrowset application can be started in three different ways
 * 1. Via mainframe JCL
	...
	public static void main(String[] args) {
		...
		if (StringUtils.hasText(appType) && WebApplicationType.NONE.equals(WebApplicationType.valueOf(appType.trim().toUpperCase())))
            ...
            GlobalExecutorCtx.load(CFApplication.class, context);
		 		...
		 		Dbrowset dbrowset = (Dbrowset) context.getBean("dbrowset");
        	    dbrowset.process(new GlobalExecutorCtx().getContext(DbrowsetCtx.class)); /*  Dbrowset requires no input parameters  */
