...
/**
 * The Db2varch application can be started in three different ways
 * 1. Via mainframe JCL
	...
	public static void main(String[] args) {
		...
		if (StringUtils.hasText(appType) && WebApplicationType.NONE.equals(WebApplicationType.valueOf(appType.trim().toUpperCase())))
            ...
            GlobalExecutorCtx.load(CFApplication.class, context);
		 		...
		 		Db2varch db2varch = (Db2varch) context.getBean("db2varch");
        	    db2varch.process(new GlobalExecutorCtx().getContext(Db2varchCtx.class)); /*  Db2varch requires no input parameters  */
