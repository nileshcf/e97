...
/**
 * The Dlcrntof application can be started in three different ways
 * 1. Via mainframe JCL
	...
	public static void main(String[] args) {
		...
		if (StringUtils.hasText(appType) && WebApplicationType.NONE.equals(WebApplicationType.valueOf(appType.trim().toUpperCase())))
            ...
            GlobalExecutorCtx.load(CFApplication.class, context);
		 		...
		 		Dlcrntof dlcrntof = (Dlcrntof) context.getBean("dlcrntof");
        	    dlcrntof.process(new GlobalExecutorCtx().getContext(DlcrntofCtx.class)); /*  Dlcrntof requires no input parameters  */
