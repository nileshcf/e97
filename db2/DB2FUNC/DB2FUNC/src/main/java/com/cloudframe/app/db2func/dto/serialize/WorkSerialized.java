...
public class WorkSerialized  extends Field { 
   ...
   protected int checkLoc1MaxLimit(long number) {
	   return (int)checkMaxLimit(number, true/*isSigned*/,4/*dataLen*/);
   }
     int localDsnpnmCounter = -1;
     public boolean isDsnpnmModified() {
         int sharedCounter = shareString.getSerializedField().getModifiedCounter(); 
         boolean hasModified = localDsnpnmCounter != sharedCounter;
         localDsnpnmCounter = sharedCounter; return hasModified;
     }

   protected char[] checkDsnpnmConstraints(char[] value) {
   			return super.checkConstraints(value , 386 ,false, false);
   }
     int localDsncnmCounter = -1;
     public boolean isDsncnmModified() {
         int sharedCounter = shareString.getSerializedField().getModifiedCounter(); 
         boolean hasModified = localDsncnmCounter != sharedCounter;
         localDsncnmCounter = sharedCounter; return hasModified;
     }

   protected char[] checkDsncnmConstraints(char[] value) {
