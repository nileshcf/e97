...
public class Db2hst1RepositoryImpl implements Db2hst1Repository {
    Logger logger = LoggerFactory.getLogger(Db2hst1RepositoryImpl.class);
...
// methodStart:selectTemp06
    
/**
******        SELECT EMP_SAL                                       
******        INTO :HV-EMP-SAL                                     
******        FROM CLOUDFRM.TEMP_06                                
******       WHERE EMP_SAL BETWEEN :HV-EMP-FROM                    
******                AND :HV-EMP-THRU                             
******       FETCH FIRST 1 ROW ONLY                                
**/
    public void selectTemp06(Sqlca sqlca, Work work, Dcltemp06 dcltemp06) throws Exception {
try {
	// SELECT 
	// reset SQLCODE
	sqlca.setSqlcode(0);
	...
	// execute SQLJ query
	resultSet = db2Base.startQuery(1/* SQL Sequence*/, CONSTANTS.LITERAL_DB2HST1)
.withSql(SQLS.SQL_1014538080)
				.withInputs(work.getHvEmpFrom(),work.getHvEmpThru())
 ...
 if (resultObj != null) {
   dcltemp06.setHvEmpSal( resultObj.toString().toCharArray() );
