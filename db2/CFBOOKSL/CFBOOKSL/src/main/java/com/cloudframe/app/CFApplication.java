...
/**
 * The Cfbooksl application can be started in three different ways
 * 1. Via mainframe JCL
	...
	public static void main(String[] args) {
		...
		if (StringUtils.hasText(appType) && WebApplicationType.NONE.equals(WebApplicationType.valueOf(appType.trim().toUpperCase())))
            ...
            GlobalExecutorCtx.load(CFApplication.class, context);
		 		...
		 		Cfbooksl cfbooksl = (Cfbooksl) context.getBean("cfbooksl");
        	    cfbooksl.process(new GlobalExecutorCtx().getContext(CfbookslCtx.class)); /*  Cfbooksl requires no input parameters  */
