...
*/
  ...
  public class Cfbooksl extends CommonProcess {
  
  Logger logger = LoggerFactory.getLogger(Cfbooksl.class);
...
//  SELECT BOOK_ID , RTRIM ( TITLE ) , TOTAL_PAGES , RATING , ISBN , PUBLISHED_DATE , PUBLISHER_ID FROM BOOKS ORDER BY BOOK_ID ASC WITH UR
          cfbookslRepository.openBookscurCfbooksl(sqlca);
...
//  CLOSE BOOKSCUR
          cfbookslRepository.closeBookscurCfbooksl(sqlca);
...
//  FETCH BOOKSCUR INTO ? , ? , ? , ? , ? , ? , ?
          cfbookslRepository.fetchBookscurCfbooksl(sqlca,nullvars,dclbooks);
