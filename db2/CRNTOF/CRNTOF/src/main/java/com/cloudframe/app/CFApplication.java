...
/**
 * The Crntof application can be started in three different ways
 * 1. Via mainframe JCL
	...
	public static void main(String[] args) {
		...
		if (StringUtils.hasText(appType) && WebApplicationType.NONE.equals(WebApplicationType.valueOf(appType.trim().toUpperCase())))
            ...
            GlobalExecutorCtx.load(CFApplication.class, context);
		 		...
		 		Crntof crntof = (Crntof) context.getBean("crntof");
        	    crntof.process(new GlobalExecutorCtx().getContext(CrntofCtx.class)); /*  Crntof requires no input parameters  */
