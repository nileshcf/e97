...
/**
 * The Sqlcount application can be started in three different ways
 * 1. Via mainframe JCL
	...
	public static void main(String[] args) {
		...
		if (StringUtils.hasText(appType) && WebApplicationType.NONE.equals(WebApplicationType.valueOf(appType.trim().toUpperCase())))
            ...
            GlobalExecutorCtx.load(CFApplication.class, context);
		 		...
		 		Sqlcount sqlcount = (Sqlcount) context.getBean("sqlcount");
        	    sqlcount.process(new GlobalExecutorCtx().getContext(SqlcountCtx.class)); /*  Sqlcount requires no input parameters  */
