...
public class McgcmsSort003OutputWriter extends BaseProcess implements Tasklet, StepExecutionListener {

	private final Logger logger = LoggerFactory.getLogger(McgcmsSort003OutputWriter.class);
	@Autowired
	...
	List<McgcmsSort003Keys> sortRecKeys = null;
...
logger.info("STEPNAME: MCGCMSSORT003 Execution completed, number of RECORDS - IN: {},  OUT: {}", sortDetail.getRecInCounter(), recordCount);
	...
	private byte[] getSortedRecord(int index) {
		try {
			McgcmsSort003Keys k = sortRecKeys.get(index);
