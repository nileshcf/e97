...
public class McgcmsSort011OutputWriter extends BaseProcess implements Tasklet, StepExecutionListener {

	private final Logger logger = LoggerFactory.getLogger(McgcmsSort011OutputWriter.class);
	@Autowired
	...
	List<McgcmsSort011Keys> sortRecKeys = null;
	...
	private byte[] getSortedRecord(int index) {
		try {
			McgcmsSort011Keys k = sortRecKeys.get(index);
...
private void addTotSumRecord(int index, int trailerSwtch) throws Exception {
		try {
		
		McgcmsSort011Keys recKey = sortRecKeys.get(index);
			byte[] recBytes = readFromBuffer(recKey.getSortRecPos(), recKey.getSortRecLen());
		switch(trailerSwtch) {
            case 0: 			
			     if (tot_T10020 == null) {
