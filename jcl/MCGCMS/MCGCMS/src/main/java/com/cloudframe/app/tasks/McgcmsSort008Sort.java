...
import com.cloudframe.app.sort.file.McgcmsSort008SortOut;
import org.springframework.beans.factory.annotation.Value;
import com.cloudframe.app.sort.file.McgcmsSort008SortIn0;
import java.io.File;
...
import com.cloudframe.app.sort.McgcmsSort008Detail;
...
import com.cloudframe.app.sort.file.McgcmsSort008SortIn1;
...
public class McgcmsSort008Sort extends BaseProcess implements Tasklet, StepExecutionListener, InitializingBean {
	private final Logger logger = LoggerFactory.getLogger(McgcmsSort008Sort.class);
	@Autowired
	CFStepHandler cfStepHandler;

	@Autowired
	@Qualifier("batch_mcgcmssort008")
	McgcmsSort008Detail sortDetail;
	
	@Autowired
	@Qualifier("McgcmsSort008SortOut")
	McgcmsSort008SortOut sortOut;
	

	
	@Autowired
	@Qualifier("McgcmsSort008SortIn0")
	McgcmsSort008SortIn0 sortIn0;
	
	@Autowired
	@Qualifier("McgcmsSort008SortIn1")
	McgcmsSort008SortIn1 sortIn1;
	...
	byte[] sortoutCond01 = convertChar2EbcdicBytes(" ".toCharArray());
	...
	List<McgcmsSort008Keys> sortRecKeys = null;
	...
	public RepeatStatus execute(StepContribution stepContribution, ChunkContext chunkContext) throws Exception {
		...
		for (int i = 0; i < sortRecKeys.size(); i++) {
		  ...
		  if (!((Field.compareBytes(outRecBytes,sortoutCond01,0,0,1,1) == 0))){
...
logger.info("STEPNAME: MCGCMSSORT008 Execution completed, number of RECORDS - IN: {},  OUT: {}", sortDetail.getRecInCounter(), recordCount);
		sortOut.close();
	...
	private byte[] getSortedRecord(int index) {
		try {
			McgcmsSort008Keys k = sortRecKeys.get(index);
           if (isWriteInTempFile()) {
...
sortDecider.addSortKeysInfo("Key3"/* name */, JNKEY_POS3/* pos */, JNKEY_LEN3/* len */, false/* descending */,"CH" /* type */,sortProperties);
    ...
    public void reformatbeforeStep() {
     ...
     public void addRecordBytes4Sorting(byte[] recordByte, int byteLen) { 
		List<McgcmsSort008Keys> sortRecKeys = sortDetail.getSortRecKeys();
		McgcmsSort008Keys relKey = new McgcmsSort008Keys();
