...
public class McgcmsSort023OutputWriter extends BaseProcess implements Tasklet, StepExecutionListener {

	private final Logger logger = LoggerFactory.getLogger(McgcmsSort023OutputWriter.class);
	@Autowired
	...
	List<McgcmsSort023Keys> sortRecKeys = null;
...
logger.info("STEPNAME: MCGCMSSORT023 Execution completed, number of RECORDS - IN: {},  OUT: {}", sortDetail.getRecInCounter(), recordCount);
	...
	private byte[] getSortedRecord(int index) {
		try {
			McgcmsSort023Keys k = sortRecKeys.get(index);
