...
public class BatchConfiguration {
   ...
   private static final String SORT = "SORT";
    ...
    @Bean
    @JobScope
    public Sort036Step020InputReader sort036Step020InputReader() {
    	return new Sort036Step020InputReader();
    }
    
    @Bean
    @JobScope
    public Sort036Step020Processor sort036Step020Processor() {
    	return new Sort036Step020Processor();
    }
    
    @Bean
    @JobScope
    public Sort036Step020OutputWriter sort036Step020OutputWriter() {
    	return new Sort036Step020OutputWriter();
    }
    
    @Bean
    @JobScope
    protected Step readSort036Step020SortInput(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
    	return new StepBuilder("readSort036Step020SortInput",jobRepository).tasklet(sort036Step020InputReader(), transactionManager).build();  
    }
    
    @Bean
    @JobScope
    protected Step processSort036Step020Sort(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
    	return new StepBuilder("sortSort036Step020Process",jobRepository).tasklet(sort036Step020Processor(), transactionManager).build();  
    }
    
    @Bean
    @JobScope
    protected Step writeSort036Step020SortOutput(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
    	return new StepBuilder("writeSort036Step020SortOutput",jobRepository).tasklet(sort036Step020OutputWriter(), transactionManager).build();  
    }
  ...
  /*  
    ...
    public Flow step020Executor(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
       return new FlowBuilder<SimpleFlow>("step020Executor")
             ...
             .to(readSort036Step020SortInput(jobRepository, transactionManager))
             .next(processSort036Step020Sort(jobRepository, transactionManager))
             .next(writeSort036Step020SortOutput(jobRepository, transactionManager))
