...
public class SyncsortSort16OutputWriter extends BaseProcess implements Tasklet, StepExecutionListener {

	private final Logger logger = LoggerFactory.getLogger(SyncsortSort16OutputWriter.class);
	@Autowired
	...
	List<SyncsortSort16Keys> sortRecKeys = null;
...
logger.info("STEPNAME: SYNCSORTSORT16 Execution completed, number of RECORDS - IN: {},  OUT: {}", sortDetail.getRecInCounter(), recordCount);
	...
	private byte[] getSortedRecord(int index) {
		try {
			SyncsortSort16Keys k = sortRecKeys.get(index);
           if (isWriteInTempFile()) {
