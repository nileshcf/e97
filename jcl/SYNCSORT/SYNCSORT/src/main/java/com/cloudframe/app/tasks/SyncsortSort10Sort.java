...
import com.cloudframe.app.sort.file.SyncsortSort10SortIn;
import com.cloudframe.app.data.Field;
import org.springframework.batch.core.StepContribution;
import org.springframework.batch.core.scope.context.ChunkContext;

import com.cloudframe.app.sort.SyncsortSort10Detail;
import org.springframework.batch.repeat.RepeatStatus;
import com.cloudframe.app.sort.file.SyncsortSort10SortIn;
import com.cloudframe.app.sort.file.SyncsortSort10SortOut;
...
import com.cloudframe.app.sort.file.records.SyncsortSort10Keys;
...
public class SyncsortSort10Sort extends BaseProcess implements Tasklet, StepExecutionListener, InitializingBean {
	private final Logger logger = LoggerFactory.getLogger(SyncsortSort10Sort.class);
	@Autowired
	CFStepHandler cfStepHandler;

	@Autowired
	@Qualifier("batch_syncsortsort10")
	SyncsortSort10Detail sortDetail;
	
	@Autowired
	@Qualifier("SyncsortSort10SortOut")
	SyncsortSort10SortOut sortOut;
	

	
	@Autowired
	@Qualifier("SyncsortSort10SortIn")
	SyncsortSort10SortIn sortIn;
	...
	List<SyncsortSort10Keys> sortRecKeys = null;
/**
...
public void afterPropertiesSet() throws Exception {
    ...
    byte[] inrecWhen1 = convertChar2EbcdicBytes("USD".toCharArray());
    byte[] inrecOverlay1 = convertChar2EbcdicBytes("USD".toCharArray());
	@Override
	...
	public RepeatStatus execute(StepContribution stepContribution, ChunkContext chunkContext) throws Exception {
		...
		cfStepHandler.updateSortStepExecution("sort10");
...
logger.info("STEPNAME: SYNCSORTSORT10 Execution completed, number of RECORDS - IN: {},  OUT: {}", sortDetail.getRecInCounter(), recordCount);
		sortOut.close();
	...
	private byte[] getSortedRecord(int index) {
		try {
			SyncsortSort10Keys k = sortRecKeys.get(index);
	...
	public void addRecordBytes4Sorting(byte[] recordByte, int byteLen) {
		Set<SyncsortSort10Keys> setRecKeys = sortDetail.getSortDistRecKeys();
		SyncsortSort10Keys relKey = new SyncsortSort10Keys();
    ...
    private byte[] getInrecRecord(byte[] record) {  
        if (Field.compareBytes(record, inrecWhen1, 25, 0, 3, 3)  != 0) {
        
         copyRecordBytes(inrecOverlay1,record, 0, 25, inrecOverlay1.length); }
	...
	public void sortProcessor() throws Exception {
		...
		sortDetail.setSortComp(
                   SyncsortSort10Keys.getKeyCharacter1Comparator(true/*isAscending*/)               
		);
	  doSort();
    }	
	public void doSort() throws Exception {
		// Sort algorithm here
		Comparator<SyncsortSort10Keys> sortComp = sortDetail.getSortComp();
