...
import com.cloudframe.app.sort.SyncsortSort9Detail;
import com.cloudframe.app.sort.file.SyncsortSort9SortOut;
import com.cloudframe.app.data.Field;
import com.cloudframe.app.sort.file.SyncsortSort9SortIn;
...
public class SyncsortSort9Sort extends BaseProcess implements Tasklet, StepExecutionListener, InitializingBean {
	private final Logger logger = LoggerFactory.getLogger(SyncsortSort9Sort.class);
	@Autowired
	CFStepHandler cfStepHandler;

	@Autowired
	@Qualifier("batch_syncsortsort9")
	SyncsortSort9Detail sortDetail;
	
	@Autowired
	@Qualifier("SyncsortSort9SortOut")
	SyncsortSort9SortOut sortOut;
	

	
	@Autowired
	@Qualifier("SyncsortSort9SortIn")
	SyncsortSort9SortIn sortIn;
	...
	List<SyncsortSort9Keys> sortRecKeys = null;
/**
...
logger.info("STEPNAME: SYNCSORTSORT9 Execution completed, number of RECORDS - IN: {},  OUT: {}", sortDetail.getRecInCounter(), recordCount);
		sortOut.close();
	...
	private byte[] getSortedRecord(int index) {
		try {
			SyncsortSort9Keys k = sortRecKeys.get(index);
...
copyRecordBytes(repBytes, recBytes, OUTREC_POS2,OUTREC_DES_POS2,OUTREC_LEN2);
	...
	public void addRecordBytes4Sorting(byte[] recordByte, int byteLen) {
		Set<SyncsortSort9Keys> setRecKeys = sortDetail.getSortDistRecKeys();
		SyncsortSort9Keys relKey = new SyncsortSort9Keys();
	...
	public void sortProcessor() throws Exception {
		...
		sortDetail.setSortComp(
                   SyncsortSort9Keys.getKeyCharacter1Comparator(true/*isAscending*/)               
		);
	  doSort();
    }	
	public void doSort() throws Exception {
		// Sort algorithm here
		Comparator<SyncsortSort9Keys> sortComp = sortDetail.getSortComp();
