...
import com.cloudframe.app.sort.McsfsortSort015Detail;
import com.cloudframe.app.data.Field;
import com.cloudframe.app.sort.model.InMemoryKeySortKeyObject;
import com.cloudframe.app.sort.file.records.McsfsortSort015Keys;
import com.cloudframe.app.sort.file.McsfsortSort015SortIn1;
...
import com.cloudframe.app.sort.file.McsfsortSort015SortOut;
import java.io.RandomAccessFile;
import java.util.Collections;
import org.springframework.beans.factory.InitializingBean;
import com.cloudframe.app.process.BaseProcess;
import com.cloudframe.app.sort.file.McsfsortSort015SortIn0;
...
public class McsfsortSort015Sort extends BaseProcess implements Tasklet, StepExecutionListener, InitializingBean {
	private final Logger logger = LoggerFactory.getLogger(McsfsortSort015Sort.class);
	@Autowired
	CFStepHandler cfStepHandler;

	@Autowired
	@Qualifier("batch_mcsfsortsort015")
	McsfsortSort015Detail sortDetail;
	
	@Autowired
	@Qualifier("McsfsortSort015SortOut")
	McsfsortSort015SortOut sortOut;
	

	
	@Autowired
	@Qualifier("McsfsortSort015SortIn0")
	McsfsortSort015SortIn0 sortIn0;
	
	@Autowired
	@Qualifier("McsfsortSort015SortIn1")
	McsfsortSort015SortIn1 sortIn1;
	...
	List<McsfsortSort015Keys> sortRecKeys = null;
...
logger.info("STEPNAME: MCSFSORTSORT015 Execution completed, number of RECORDS - IN: {},  OUT: {}", sortDetail.getRecInCounter(), recordCount);
		sortOut.close();
	...
	private byte[] getSortedRecord(int index) {
		try {
			McsfsortSort015Keys k = sortRecKeys.get(index);
           if (isWriteInTempFile()) {
...
private void processJoinKeys2() throws Exception {	
	...
	public void processReformat() throws Exception {
        ...
        List<InMemoryKeySortKeyObject> joinRecKeys2 =  sortDetail.getJn2RecKeys().stream()
				.filter(jk2 -> !sortDetail.getJn1RecKeys().contains(jk2)).collect(Collectors.toList());
    	...
    	for (int i=0; i<joinRecKeys1.size(); i++) {
			...
			outRecBytes = new byte[REFORMAT_KEY_LEN];
			copyRecordBytes(fillEmptySpaces, outRecBytes, 0, 0, REFORMAT_KEY_LEN);
	    	byte[] key2 = getJnReturnRecord(joinRecKeys2.get(i),1);
		...
		logger.info("STEPNAME: SORT015 Joinkey Formatter execution completed, number of RECORDS - IN: {},  OUT: {}", sortDetail.getRecInCounter(), reformatCount);
		clearMemMapFiles();
    ...
    public void reformatbeforeStep() {
     ...
     public void addRecordBytes4Sorting(byte[] recordByte, int byteLen) { 
		List<McsfsortSort015Keys> sortRecKeys = sortDetail.getSortRecKeys();
		McsfsortSort015Keys relKey = new McsfsortSort015Keys();
