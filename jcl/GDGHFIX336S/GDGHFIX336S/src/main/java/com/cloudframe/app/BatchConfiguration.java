...
public class BatchConfiguration {
    ...
    private static final String JOBNAME = "GDGHFIX336S";
   ...
   private static final String SORT = "SORT";
    ...
    private static final String GDGPGM2_BEAN = "gdgpgm2";
    ...
    public Job job(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
	    return new JobBuilder(JOBNAME, jobRepository)
			...
			.next(step020(jobRepository, transactionManager)) /*  */
    ...
    @Bean
    @JobScope
    public Gdghfix336sStep001InputReader oldnewStep001InputReader() {
    	return new Gdghfix336sStep001InputReader();
    }
    
    @Bean
    @JobScope
    public Gdghfix336sStep001Processor oldnewStep001Processor() {
    	return new Gdghfix336sStep001Processor();
    }
    
    @Bean
    @JobScope
    public Gdghfix336sStep001OutputWriter oldnewStep001OutputWriter() {
    	return new Gdghfix336sStep001OutputWriter();
    }
    
    @Bean
    @JobScope
    protected Step readGdghfix336sStep001SortInput(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
    	return new StepBuilder("readGdghfix336sStep001SortInput",jobRepository).tasklet(oldnewStep001InputReader(), transactionManager).build();  
    }
    
    @Bean
    @JobScope
    protected Step processGdghfix336sStep001Sort(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
    	return new StepBuilder("sortGdghfix336sStep001Process",jobRepository).tasklet(oldnewStep001Processor(), transactionManager).build();  
    }
    
    @Bean
    @JobScope
    protected Step writeGdghfix336sStep001SortOutput(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
    	return new StepBuilder("writeGdghfix336sStep001SortOutput",jobRepository).tasklet(oldnewStep001OutputWriter(), transactionManager).build();  
    }
  ...
  /*  
    ...
    public Flow step001Executor(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
       return new FlowBuilder<SimpleFlow>("step001Executor")
             ...
             .to(readGdghfix336sStep001SortInput(jobRepository, transactionManager))
             .next(processGdghfix336sStep001Sort(jobRepository, transactionManager))
             .next(writeGdghfix336sStep001SortOutput(jobRepository, transactionManager))
    ...
    public Step step010(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
     ...
     int rc = 0;
 	   try { 
 		    	  		rc = ((Oldnew)cfStepHandler.initAndGetProcessBean(STEP010_STEP,OLDNEW_BEAN)).process();
...
cfStepHandler.setAbendCode(cfe);
    ...
    /*
     * STEP020 GDGPGM2 starts here
     */
    
    private static final String STEP020_STEP = "step020";
    
    @Bean
    public Step step020(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
    
      return new StepBuilder(STEP020_STEP,jobRepository).tasklet(
    		new Tasklet() {
     ...
     int rc = 0;
 	   try { 
 		    	  		rc = ((Gdgpgm2)cfStepHandler.initAndGetProcessBean(STEP020_STEP,GDGPGM2_BEAN)).process();
 } catch (CFException cfe) { 
 if (cfe.isTerminated()) {
 rc = cfe.getCode();
  } else {
...
cfStepHandler.setAbendCode(cfe);
 		    	  		...
 		    	  		rcStatus+=rc;
 		    	   		cfStepHandler.postStepExecution(STEP020_STEP, rc ,GDGPGM2_BEAN);
                      } 
    			   cfStepHandler.updateStepExecution(STEP020_STEP, chunkContext , rcStatus , COMPLETED_STATUS);
 		          return RepeatStatus.FINISHED;
 		     } 
