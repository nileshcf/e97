...
public class BatchConfiguration {
    ...
    public Step writevb(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
     ...
     int rc = 0;
 	   try { 
 		    	  		rc = ((Iovb32k)cfStepHandler.initAndGetProcessBean(WRITEVB_STEP,IOVB32K_BEAN)).setParameter(new GlobalExecutorCtx().getContext("IOVB32K"), cfStepHandler.getParm(writevbIovb32kParm));
...
cfStepHandler.setAbendCode(cfe);
    ...
    public Step readvb(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
     ...
     int rc = 0;
 	   try { 
 		    	  		rc = ((Iovb32k)cfStepHandler.initAndGetProcessBean(READVB_STEP,IOVB32K_BEAN)).setParameter(new GlobalExecutorCtx().getContext("IOVB32K"), cfStepHandler.getParm(readvbIovb32kParm));
 } catch (CFException cfe) { 
 if (cfe.isTerminated()) {
 rc = cfe.getCode();
  } else {
