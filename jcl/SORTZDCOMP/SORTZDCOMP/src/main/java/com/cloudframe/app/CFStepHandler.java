...
public class CFStepHandler implements InitializingBean {
    ...
    private static final String PRIP7001_STEP030 = "prip7001_step030";
...
*/
  ...
  public void handleStepOverrides(String stepName) throws CFException {
        switch(stepName) {
            case "prip7001_step030":  /* setting the disp position for file(s) info for this step(PRIP7001_STEP030). */
                         SpringContextHandler.setFileDispPosition("SortzdcompPrip7001_step030SortIn","SortzdcompPrip7001_step030SortIn.DISP");
                         SpringContextHandler.setFileDispPosition("SortzdcompPrip7001_step030SortOut","SortzdcompPrip7001_step030SortOut.DISP");
            break;
                default: break;
        }
}
...
*/ 
    ...
    public void printExecutionSummary() {
      ...
      if(abendList.isEmpty()) {
          ...
          logger.info("JOB : SORTZDCOMP ENDED -  {}", formattedJobRc);
      } else { 
          logger.info("JOB : SORTZDCOMP ENDED -  ABEND={}", abendCode);
      } 
