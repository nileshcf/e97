  ...
  public void handleStepOverrides(String stepName) throws CFException {
        switch(stepName) {
            case "step01":  /* setting the appropriate file(s) info for this step(STEP01). */
 		    	   		SpringContextHandler.setFileDispPosition("tstcdpag_inputFile", "TSTCDPAG.InputFile.DISP");
 		    	   		SpringContextHandler.setFileDispPosition("tstcdpag_outputFile", "TSTCDPAG.OutputFile.DISP");
            break;
                default: break;
        }
}
...
*/ 
    ...
    public void printExecutionSummary() {
      ...
      if(abendList.isEmpty()) {
          ...
          logger.info("JOB : TSTCDPAG ENDED -  {}", formattedJobRc);
      } else { 
          logger.info("JOB : TSTCDPAG ENDED -  ABEND={}", abendCode);
      } 
