...
public class BatchConfiguration {
    ...
    private static final String JOBNAME = "IDCPRN9A";
   ...
   private static final String IDCAMS = "IDCAMS";
    ...
    public Job job(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
      GlobalExecutorCtx.load(BatchConfiguration.class, context);
	    return new JobBuilder(JOBNAME, jobRepository)
			...
			.next(vsamdel(jobRepository, transactionManager)) /*  */
    ...
    /*
     * VSAMDEL IDCAMS starts here
     */
    
    private static final String VSAMDEL_STEP = "vsamdel";
    
    @Bean
    public Step vsamdel(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
    
      return new StepBuilder(VSAMDEL_STEP,jobRepository).tasklet(
    		new Tasklet() {
    
 		       @Override
 		       public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) throws Exception {
                      String rcStatus = "";
                      if (cfStepHandler.stepDecider(VSAMDEL_STEP)) {
 		    	   		int rc =0;
                  try {
 		    	   		rc = CFVsam.deleteCluster("CLOUDFRM_IDCAMS_PRINT_VSAMKSDS.json"/*Cluster json*/);
                          } catch (CFException e) {
 		    	  		...
 		    	  		rcStatus+=rc;
 		    	   		cfStepHandler.postStepExecution(VSAMDEL_STEP, rc ,IDCAMS);
                      } 
    			   cfStepHandler.updateStepExecution(VSAMDEL_STEP, chunkContext , rcStatus , COMPLETED_STATUS);
 		          return RepeatStatus.FINISHED;
 		     } 
