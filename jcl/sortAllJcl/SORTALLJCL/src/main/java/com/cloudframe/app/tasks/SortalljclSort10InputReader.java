...
public class SortalljclSort10InputReader extends BaseProcess implements Tasklet, StepExecutionListener, InitializingBean {
	private final Logger logger = LoggerFactory.getLogger(SortalljclSort10InputReader.class);
/**
...
public void afterPropertiesSet() throws Exception {
    ...
    byte[] when1 = convertChar2EbcdicBytes("USD".toCharArray());
    byte[] overlay1 = convertChar2EbcdicBytes("USD".toCharArray());
	...
	public void addRecordBytes4Sorting(byte[] recordByte, int byteLen) {
		Set<SortalljclSort10Keys> setRecKeys = sortDetail.getSortDistRecKeys();
		SortalljclSort10Keys relKey = new SortalljclSort10Keys();
    ...
    private byte[] getInrecRecord(byte[] record) {  
        if (Field.compareBytes(record, when1, 25, 0, 3, 3)  != 0) {
        
         copyRecordBytes(overlay1,record, 0, 25, overlay1.length); }
