...
import com.cloudframe.app.sort.file.SortalljclSort12SortIn;
import java.io.File;
import java.util.HashSet;
import java.util.stream.Collectors;
import com.cloudframe.app.data.Field;
import com.cloudframe.app.sort.file.SortalljclSort12SortOut;
...
import com.cloudframe.app.sort.SortalljclSort12Detail;
import org.springframework.beans.factory.annotation.Autowired;
import com.cloudframe.app.sort.file.records.SortalljclSort12Keys;
...
public class SortalljclSort12Sort extends BaseProcess implements Tasklet, StepExecutionListener, InitializingBean {
	private final Logger logger = LoggerFactory.getLogger(SortalljclSort12Sort.class);
	@Autowired
	CFStepHandler cfStepHandler;

	@Autowired
	@Qualifier("batch_sortalljclsort12")
	SortalljclSort12Detail sortDetail;
	
	@Autowired
	@Qualifier("SortalljclSort12SortOut")
	SortalljclSort12SortOut sortOut;
	

	
	@Autowired
	@Qualifier("SortalljclSort12SortIn")
	SortalljclSort12SortIn sortIn;
	...
	List<SortalljclSort12Keys> sortRecKeys = null;
/**
...
public void afterPropertiesSet() throws Exception {
	...
	static final int KEY_LEN0 = 8;
     byte[] filter1 = convertChar2EbcdicBytes("1".toCharArray());
	@Override
	...
	public RepeatStatus execute(StepContribution stepContribution, ChunkContext chunkContext) throws Exception {
		...
		cfStepHandler.updateSortStepExecution("sort12");
...
logger.info("STEPNAME: SORTALLJCLSORT12 Execution completed, number of RECORDS - IN: {},  OUT: {}", sortDetail.getRecInCounter(), recordCount);
		sortOut.close();
	...
	private byte[] getSortedRecord(int index) {
		try {
			SortalljclSort12Keys k = sortRecKeys.get(index);
	...
	public void addRecordBytes4Sorting(byte[] recordByte, int byteLen) {
		Set<SortalljclSort12Keys> setRecKeys = sortDetail.getSortDistRecKeys();
		SortalljclSort12Keys relKey = new SortalljclSort12Keys();
    ...
    protected void addSortRecordBytes(byte[] record){ 
	   if(!((Field.compareBytes(record,filter1,0,0,1,1) == 0))) {
	...
	public void sortProcessor() throws Exception {
		...
		sortDetail.setSortComp(
                   SortalljclSort12Keys.getKeyCharacter1Comparator(true/*isAscending*/)               
		);
	  doSort();
    }	
	public void doSort() throws Exception {
		// Sort algorithm here
		Comparator<SortalljclSort12Keys> sortComp = sortDetail.getSortComp();
