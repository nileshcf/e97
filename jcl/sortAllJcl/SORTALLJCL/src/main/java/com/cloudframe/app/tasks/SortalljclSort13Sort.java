...
import com.cloudframe.app.sort.file.SortalljclSort13SortIn;
import org.springframework.batch.core.ExitStatus;
import org.springframework.beans.factory.annotation.Value;
import java.io.File;
import java.util.HashSet;
import com.cloudframe.app.sort.file.SortalljclSort13SortOut;
import java.util.stream.Collectors;
import com.cloudframe.app.data.Field;
import com.cloudframe.app.sort.file.records.SortalljclSort13Keys;
import org.springframework.batch.core.StepContribution;
import org.springframework.batch.core.scope.context.ChunkContext;

import org.springframework.batch.repeat.RepeatStatus;
import java.util.List;
import com.cloudframe.app.sort.SortalljclSort13Detail;
...
public class SortalljclSort13Sort extends BaseProcess implements Tasklet, StepExecutionListener, InitializingBean {
	private final Logger logger = LoggerFactory.getLogger(SortalljclSort13Sort.class);
	@Autowired
	CFStepHandler cfStepHandler;

	@Autowired
	@Qualifier("batch_sortalljclsort13")
	SortalljclSort13Detail sortDetail;
	
	@Autowired
	@Qualifier("SortalljclSort13SortOut")
	SortalljclSort13SortOut sortOut;
	

	
	@Autowired
	@Qualifier("SortalljclSort13SortIn")
	SortalljclSort13SortIn sortIn;
	...
	List<SortalljclSort13Keys> sortRecKeys = null;
/**
...
public void afterPropertiesSet() throws Exception {
	...
	public RepeatStatus execute(StepContribution stepContribution, ChunkContext chunkContext) throws Exception {
		...
		cfStepHandler.updateSortStepExecution("sort13");
...
logger.info("STEPNAME: SORTALLJCLSORT13 Execution completed, number of RECORDS - IN: {},  OUT: {}", sortDetail.getRecInCounter(), recordCount);
		sortOut.close();
	...
	private byte[] getSortedRecord(int index) {
		try {
			SortalljclSort13Keys k = sortRecKeys.get(index);
	...
	public void addRecordBytes4Sorting(byte[] recordByte, int byteLen) {
		Set<SortalljclSort13Keys> setRecKeys = sortDetail.getSortDistRecKeys();
		SortalljclSort13Keys relKey = new SortalljclSort13Keys();
    ...
    protected void addSortRecordBytes(byte[] record){ 
	   if(!((Field.compareByte(record[0],(byte)0xF1) <= 0))) {
	...
	public void sortProcessor() throws Exception {
		...
		sortDetail.setSortComp(
                   SortalljclSort13Keys.getKeyCharacter1Comparator(true/*isAscending*/)               
		);
	  doSort();
    }	
	public void doSort() throws Exception {
		// Sort algorithm here
		Comparator<SortalljclSort13Keys> sortComp = sortDetail.getSortComp();
