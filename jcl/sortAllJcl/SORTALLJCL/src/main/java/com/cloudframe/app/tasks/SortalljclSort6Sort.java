...
import com.cloudframe.app.sort.file.SortalljclSort6SortOut;
import org.springframework.beans.factory.annotation.Value;
import java.io.File;
import java.util.HashSet;
import java.util.stream.Collectors;
import com.cloudframe.app.sort.file.SortalljclSort6SortIn;
import org.springframework.batch.core.StepContribution;
import org.springframework.batch.core.scope.context.ChunkContext;
import com.cloudframe.app.sort.file.records.SortalljclSort6Keys;

import org.springframework.batch.repeat.RepeatStatus;
import java.util.List;
import java.io.RandomAccessFile;
import com.cloudframe.app.sort.SortalljclSort6Detail;
...
public class SortalljclSort6Sort extends BaseProcess implements Tasklet, StepExecutionListener, InitializingBean {
	private final Logger logger = LoggerFactory.getLogger(SortalljclSort6Sort.class);
	@Autowired
	CFStepHandler cfStepHandler;

	@Autowired
	@Qualifier("batch_sortalljclsort6")
	SortalljclSort6Detail sortDetail;
	
	@Autowired
	@Qualifier("SortalljclSort6SortOut")
	SortalljclSort6SortOut sortOut;
	

	
	@Autowired
	@Qualifier("SortalljclSort6SortIn")
	SortalljclSort6SortIn sortIn;
	
	
    int recordCount = 0;


    static final int OUTREC_SIZE = 9;    
	int recordLen = 0;
	...
	List<SortalljclSort6Keys> sortRecKeys = null;
/**
...
public void afterPropertiesSet() throws Exception {
	...
	public RepeatStatus execute(StepContribution stepContribution, ChunkContext chunkContext) throws Exception {
		...
		cfStepHandler.updateSortStepExecution("sort6");
...
logger.info("STEPNAME: SORTALLJCLSORT6 Execution completed, number of RECORDS - IN: {},  OUT: {}", sortDetail.getRecInCounter(), recordCount);
		sortOut.close();
	...
	private byte[] getSortedRecord(int index) {
		try {
			SortalljclSort6Keys k = sortRecKeys.get(index);
	...
	public void addRecordBytes4Sorting(byte[] recordByte, int byteLen) {
		Set<SortalljclSort6Keys> setRecKeys = sortDetail.getSortDistRecKeys();
		SortalljclSort6Keys relKey = new SortalljclSort6Keys();
	...
	public void sortProcessor() throws Exception {
		...
		sortDetail.setSortComp(
                   SortalljclSort6Keys.getKeyCharacter1Comparator(true/*isAscending*/)               
		);
	  doSort();
    }	
	public void doSort() throws Exception {
		// Sort algorithm here
		Comparator<SortalljclSort6Keys> sortComp = sortDetail.getSortComp();
