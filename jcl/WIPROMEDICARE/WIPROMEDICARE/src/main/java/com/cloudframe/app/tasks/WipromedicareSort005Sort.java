...
public class WipromedicareSort005Sort extends BaseProcess implements Tasklet, StepExecutionListener, InitializingBean {
	private final Logger logger = LoggerFactory.getLogger(WipromedicareSort005Sort.class);
	@Autowired
...
logger.info("STEPNAME: WIPROMEDICARESORT005 Execution completed, number of RECORDS - IN: {},  OUT: {}", sortDetail.getRecInCounter(), recordCount);
	...
	public void sortProcessor() throws Exception {
		...
		sortDetail.setSortComp(
                   WipromedicareSort005Keys.getKeyBinary1Comparator(false/*isAscending*/)               
                   .thenComparing(WipromedicareSort005Keys.getKeyBinary2Comparator(true/*isAscending*/))               
		);
	  doSort();
    }	
	public void doSort() throws Exception {
		// Sort algorithm here
		Comparator<WipromedicareSort005Keys> sortComp = sortDetail.getSortComp();
