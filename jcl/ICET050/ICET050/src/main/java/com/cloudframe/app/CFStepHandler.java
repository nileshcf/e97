  ...
  public void handleStepOverrides(String stepName) throws CFException {
        switch(stepName) {
            case "merge":  /* setting the disp position for file(s) info for this step(MERGE). */
                         SpringContextHandler.setFileDispPosition("Icet050MergeSortIn0","Icet050MergeSortIn[0].DISP");
                         SpringContextHandler.setFileDispPosition("Icet050MergeSortIn1","Icet050MergeSortIn[1].DISP");
            break;
                default: break;
        }
}
...
*/ 
    ...
    public void printExecutionSummary() {
      ...
      if(abendList.isEmpty()) {
          ...
          logger.info("JOB : ICET050 ENDED -  {}", formattedJobRc);
      } else { 
          logger.info("JOB : ICET050 ENDED -  ABEND={}", abendCode);
      } 
