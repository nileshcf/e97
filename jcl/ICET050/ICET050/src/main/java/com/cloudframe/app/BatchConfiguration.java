...
public class BatchConfiguration {
   ...
   private static final String ICETOOL = "ICETOOL";
    ...
    @Bean
    @JobScope
    public Icet050MergeInputReader icet050MergeInputReader() {
    	return new Icet050MergeInputReader();
    }
    
    @Bean
    @JobScope
    public Icet050MergeProcessor icet050MergeProcessor() {
    	return new Icet050MergeProcessor();
    }
    
    @Bean
    @JobScope
    public Icet050MergeOutputWriter icet050MergeOutputWriter() {
    	return new Icet050MergeOutputWriter();
    }
    
    @Bean
    @JobScope
    protected Step readIcet050MergeSortInput(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
    	return new StepBuilder("readIcet050MergeSortInput",jobRepository).tasklet(icet050MergeInputReader(), transactionManager).build();  
    }
    
    @Bean
    @JobScope
    protected Step processIcet050MergeSort(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
    	return new StepBuilder("sortIcet050MergeProcess",jobRepository).tasklet(icet050MergeProcessor(), transactionManager).build();  
    }
    
    @Bean
    @JobScope
    protected Step writeIcet050MergeSortOutput(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
    	return new StepBuilder("writeIcet050MergeSortOutput",jobRepository).tasklet(icet050MergeOutputWriter(), transactionManager).build();  
    }
  ...
  /*  
    ...
    public Flow mergeExecutor(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
       return new FlowBuilder<SimpleFlow>("mergeExecutor")
             ...
             .to(readIcet050MergeSortInput(jobRepository, transactionManager))
             .next(processIcet050MergeSort(jobRepository, transactionManager))
             .next(writeIcet050MergeSortOutput(jobRepository, transactionManager))
