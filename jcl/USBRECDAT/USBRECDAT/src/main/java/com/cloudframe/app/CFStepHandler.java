...
public class CFStepHandler implements InitializingBean {
    ...
    private static final String DATE1IN = "date1in";
    private static final String DATE1OUT = "date1out";
...
*/
  ...
  public void handleStepOverrides(String stepName) throws CFException {
        switch(stepName) {
            case "date1in":  /* setting the disp position for file(s) info for this step(DATE1IN). */
                         SpringContextHandler.setFileDispPosition("UsbrecdatDate1inSortIn","UsbrecdatDate1inSortIn.DISP");
                         SpringContextHandler.setFileDispPosition("UsbrecdatDate1inSortOut","UsbrecdatDate1inSortOut.DISP");
            break;
            case "date1out":  /* setting the disp position for file(s) info for this step(DATE1OUT). */
                         SpringContextHandler.setFileDispPosition("UsbrecdatDate1outSortIn","UsbrecdatDate1outSortIn.DISP");
                         SpringContextHandler.setFileDispPosition("UsbrecdatDate1outSortOut","UsbrecdatDate1outSortOut.DISP");
            break;
                default: break;
        }
}
...
*/ 
    ...
    public void printExecutionSummary() {
      ...
      if(abendList.isEmpty()) {
          ...
          logger.info("JOB : USBRECDAT ENDED -  {}", formattedJobRc);
      } else { 
          logger.info("JOB : USBRECDAT ENDED -  ABEND={}", abendCode);
      } 
