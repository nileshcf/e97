...
public class UsbrecdatDate1inOutputWriter extends BaseProcess implements Tasklet, StepExecutionListener {

	private final Logger logger = LoggerFactory.getLogger(UsbrecdatDate1inOutputWriter.class);
	@Autowired
	...
	List<UsbrecdatDate1inKeys> sortRecKeys = null;
...
logger.info("STEPNAME: USBRECDATDATE1IN Execution completed, number of RECORDS - IN: {},  OUT: {}", sortDetail.getRecInCounter(), recordCount);
	...
	private byte[] getSortedRecord(int index) {
		try {
			UsbrecdatDate1inKeys k = sortRecKeys.get(index);
