...
public class UsbrecdatDate1outOutputWriter extends BaseProcess implements Tasklet, StepExecutionListener {

	private final Logger logger = LoggerFactory.getLogger(UsbrecdatDate1outOutputWriter.class);
	@Autowired
	...
	List<UsbrecdatDate1outKeys> sortRecKeys = null;
...
logger.info("STEPNAME: USBRECDATDATE1OUT Execution completed, number of RECORDS - IN: {},  OUT: {}", sortDetail.getRecInCounter(), recordCount);
	...
	private byte[] getSortedRecord(int index) {
		try {
			UsbrecdatDate1outKeys k = sortRecKeys.get(index);
