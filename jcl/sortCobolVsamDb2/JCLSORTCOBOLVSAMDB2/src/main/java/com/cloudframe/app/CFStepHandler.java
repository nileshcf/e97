...
*/
    public boolean stepDecider(String stepName) {
	      ...
	      switch (stepName) {
	          ...
	          break;
	          case STEP02:
	              canExecute = abendList.isEmpty() && deciderForStep02();
	          break;
	          default:
	              canExecute = false;
	          break;
...
return canExecute;
    ...
    private boolean deciderForStep02() {
        return ( CFUtil.validateStepRc(stepMap, STEP01, 4, Cond.LE)  &&  CFUtil.validateStepRc(stepMap, TEST, 4, Cond.LE) ) /*COND CODE*/ ;
    }
...
*/
  ...
  public void handleStepOverrides(String stepName) throws CFException {
        switch(stepName) {
            case "step01":  /* setting the disp position for file(s) info for this step(STEP01). */
                         SpringContextHandler.setFileDispPosition("Jclsortcobolvsamdb2Step01SortIn","Jclsortcobolvsamdb2Step01SortIn.DISP");
                         SpringContextHandler.setFileDispPosition("Jclsortcobolvsamdb2Step01SortOut","Jclsortcobolvsamdb2Step01SortOut.DISP");
            break;
            case "test":  /* setting the appropriate file(s) info for this step(TEST). */
 		    	   		SpringContextHandler.setClusterKey("mcvsam2_sys001IrdExtrFile", "TEST.MCVSAM2");
            break;
                default: break;
        }
}
...
*/ 
    ...
    public void printExecutionSummary() {
      ...
      if(abendList.isEmpty()) {
          ...
          logger.info("JOB : JCLSORTCOBOLVSAMDB2 ENDED -  {}", formattedJobRc);
      } else { 
          logger.info("JOB : JCLSORTCOBOLVSAMDB2 ENDED -  ABEND={}", abendCode);
      } 
