...
public class BatchConfiguration {
   ...
   private static final String SORT = "SORT";
    ...
    @Bean
    @JobScope
    public Jclsortcobolvsamdb2Step01Sort cfbooksStep01Sort() {
    	return new Jclsortcobolvsamdb2Step01Sort();
    }
    
    @Bean
    @JobScope
    protected Step Jclsortcobolvsamdb2Step01Sort(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
    	return new StepBuilder("Jclsortcobolvsamdb2Step01Sort",jobRepository).tasklet(cfbooksStep01Sort(), transactionManager).build();  
    }
  ...
  /*  
    ...
    public Flow step01Executor(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
       return new FlowBuilder<SimpleFlow>("step01Executor")
             ...
             .to(Jclsortcobolvsamdb2Step01Sort(jobRepository, transactionManager))
    ...
    public Step test(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
     ...
     int rc = 0;
 	   try { 
 		    	  		rc = ((Mcvsam1)cfStepHandler.initAndGetProcessBean(TEST_STEP,MCVSAM1_BEAN)).process();
...
cfStepHandler.setAbendCode(cfe);
    ...
    public Step step02(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
     ...
     int rc = 0;
 	   try { 
 		    	  		rc = ((Cfbooks)cfStepHandler.initAndGetProcessBean(STEP02_STEP,CFBOOKS_BEAN)).process();
 } catch (CFException cfe) { 
 if (cfe.isTerminated()) {
 rc = cfe.getCode();
  } else {
