...
import com.cloudframe.app.sort.file.TranSort001SortOut;
...
public class TranSort001Sort extends BaseProcess implements Tasklet, StepExecutionListener, InitializingBean {
	private final Logger logger = LoggerFactory.getLogger(TranSort001Sort.class);
	@Autowired
	...
	@Autowired
	@Qualifier("TranSort001SortOut")
	TranSort001SortOut sortOut;
	...
	List<TranSort001Keys> sortRecKeys = null;
...
logger.info("STEPNAME: TRANSORT001 Execution completed, number of RECORDS - IN: {},  OUT: {}", sortDetail.getRecInCounter(), recordCount);
		sortOut.close();
	...
	private byte[] getSortedRecord(int index) {
		try {
			TranSort001Keys k = sortRecKeys.get(index);
           if (isWriteInTempFile()) {
...
copyTranslateEbcdic2Ascii(record, buildRec, 19,0,66); 
...
sortDecider.addSortKeysInfo("Key3"/* name */, JNKEY_POS3/* pos */, JNKEY_LEN3/* len */, false/* descending */,"CH" /* type */,sortProperties);
	...
	reformatCount++;
		...
		logger.info("STEPNAME: SORT001 Joinkey Formatter execution completed, number of RECORDS - IN: {},  OUT: {}", sortDetail.getRecInCounter(), reformatCount);
		clearMemMapFiles();
    ...
    public void reformatbeforeStep() {
     ...
     public void addRecordBytes4Sorting(byte[] recordByte, int byteLen) { 
		List<TranSort001Keys> sortRecKeys = sortDetail.getSortRecKeys();
		TranSort001Keys relKey = new TranSort001Keys();
