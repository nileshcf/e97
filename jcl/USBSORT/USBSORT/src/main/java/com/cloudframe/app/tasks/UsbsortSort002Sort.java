...
import com.cloudframe.app.sort.file.UsbsortSort002SortIn;
import java.util.List;
import com.cloudframe.app.sort.file.UsbsortSort002SortOut;
...
import com.cloudframe.app.sort.UsbsortSort002Detail;
import java.util.ArrayList;
import org.slf4j.LoggerFactory;
import com.cloudframe.app.utility.SpringContextHandler;
import com.cloudframe.app.sort.file.records.UsbsortSort002Keys;
import org.springframework.beans.factory.annotation.Qualifier;
import com.cloudframe.app.CFStepHandler;
public class UsbsortSort002Sort extends BaseProcess implements Tasklet, StepExecutionListener, InitializingBean {
	private final Logger logger = LoggerFactory.getLogger(UsbsortSort002Sort.class);
	@Autowired
	CFStepHandler cfStepHandler;

	@Autowired
	@Qualifier("batch_usbsortsort002")
	UsbsortSort002Detail sortDetail;
	
	@Autowired
	@Qualifier("UsbsortSort002SortOut")
	UsbsortSort002SortOut sortOut;
	

	
	@Autowired
	@Qualifier("UsbsortSort002SortIn")
	UsbsortSort002SortIn sortIn;
	
	
    int recordCount = 0;


    static final int OUTREC_SIZE = 29;    
	int recordLen = 0;
	


    final byte[] outrecConstLtrl0 = convertToPackedDecimalBytes(+0,6);
    final byte[] outrecConstLtrl1 = convertToPackedDecimalBytes(+0,6);
    final byte[] outrecConstLtrl2 = convertToZoneDecimalBytes(-4096,12);


	static final int OUTREC_POS1=0;
	static final int OUTREC_LEN1=5;
	static final int OUTREC_DES_POS1=0;
    private String filePath = "";

	List<UsbsortSort002Keys> sortRecKeys = null;
/**
...
logger.info("STEPNAME: USBSORTSORT002 Execution completed, number of RECORDS - IN: {},  OUT: {}", sortDetail.getRecInCounter(), recordCount);
		sortOut.close();
	...
	private byte[] getSortedRecord(int index) {
		try {
			UsbsortSort002Keys k = sortRecKeys.get(index);
	...
	private byte[] getOutRecord(int i) {
		...
		byte[] recBytes = new byte[recordLen];
            ...
            copyRecordBytes(outrecConstLtrl0, recBytes, 0,5,6); 
            copyRecordBytes(outrecConstLtrl1, recBytes, 0,11,6); 
            copyRecordBytes(outrecConstLtrl2, recBytes, 0,17,12); 
	...
	public void addRecordBytes4Sorting(byte[] recordByte, int byteLen) {
		List<UsbsortSort002Keys> sortRecKeys = sortDetail.getSortRecKeys();
		UsbsortSort002Keys relKey = new UsbsortSort002Keys();
	...
	public void sortProcessor() throws Exception {
		...
		sortDetail.setSortComp(
                   UsbsortSort002Keys.getKeyCharacter1Comparator(true/*isAscending*/)               
		);
	  doSort();
    }	
	public void doSort() throws Exception {
		// Sort algorithm here
		Comparator<UsbsortSort002Keys> sortComp = sortDetail.getSortComp();
