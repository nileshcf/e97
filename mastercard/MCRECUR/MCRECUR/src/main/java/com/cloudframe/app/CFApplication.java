...
/**
 * The Mcrecur application can be started in three different ways
 * 1. Via mainframe JCL
	...
	public static void main(String[] args) {
		...
		if (StringUtils.hasText(appType) && WebApplicationType.NONE.equals(WebApplicationType.valueOf(appType.trim().toUpperCase())))
            ...
            GlobalExecutorCtx.load(CFApplication.class, context);
		 		...
		 		Mcrecur mcrecur = (Mcrecur) context.getBean("mcrecur");
        	    mcrecur.process(new GlobalExecutorCtx().getContext(McrecurCtx.class)); /*  Mcrecur requires no input parameters  */
