...
/**
 * The Mcextend application can be started in three different ways
 * 1. Via mainframe JCL
	...
	public static void main(String[] args) {
		...
		if (StringUtils.hasText(appType) && WebApplicationType.NONE.equals(WebApplicationType.valueOf(appType.trim().toUpperCase())))
            ...
            GlobalExecutorCtx.load(CFApplication.class, context);
		 		...
		 		Mcextend mcextend = (Mcextend) context.getBean("mcextend");
        	    mcextend.process(new GlobalExecutorCtx().getContext(McextendCtx.class)); /*  Mcextend requires no input parameters  */
