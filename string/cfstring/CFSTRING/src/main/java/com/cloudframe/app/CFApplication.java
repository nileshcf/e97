...
/**
 * The Cfstring application can be started in three different ways
 * 1. Via mainframe JCL
	...
	public static void main(String[] args) {
		...
		if (StringUtils.hasText(appType) && WebApplicationType.NONE.equals(WebApplicationType.valueOf(appType.trim().toUpperCase())))
            ...
            GlobalExecutorCtx.load(CFApplication.class, context);
		 		...
		 		Cfstring cfstring = (Cfstring) context.getBean("cfstring");
        	    cfstring.process(new GlobalExecutorCtx().getContext(CfstringCtx.class)); /*  Cfstring requires no input parameters  */
