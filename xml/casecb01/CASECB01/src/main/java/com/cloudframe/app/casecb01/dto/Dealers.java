...
public class Dealers extends DealersSerialized { 
  ...
  public void setManufacturers(int index,char[] value) {
   ...
   public void initialize() {
	...
	/**
	 *	Returns the xml format of Dealers
	 *	@return xml format of Dealers
	 */
	public String getXml() throws CFException {
		StringBuilder xmlContent = new StringBuilder();
		xmlContent.append("<Dealers>");
					xmlContent.append("<DealerName>").append(CFUtil.encodeXML(getDealername())).append("</DealerName>");
				for (int index = 0;index < LOCATIONS_SIZE;index++) {
					xmlContent.append(getLocations(index).getXml());
				}
					xmlContent.append("<MakeCounter>").append(getMakecounter()).append("</MakeCounter>");
				for (int index = 0;index < MANUFACTURERS_SIZE;index++) {
					xmlContent.append(getManufacturers(index).getXml());
				}
		xmlContent.append("</Dealers>");
