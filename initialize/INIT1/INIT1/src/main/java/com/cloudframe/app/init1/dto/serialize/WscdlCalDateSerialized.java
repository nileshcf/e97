...
public class WscdlCalDateSerialized  extends Field { 
   ...
   /*  offset of each of Child Fields when serialized as a String */
            ...
            protected  int beginWcdloStartDateIso;
            protected  int beginWcdloEndDateIso;
	...
	**/
	...
	protected void init(int begin) {
	   ...
	   /*  set the offset/position of each field when this object is serialized as String */
             ...
             beginWcdloStartDateIso = getStartOffset() + 58;	// set offset for serialization
  
             beginWcdloEndDateIso = getStartOffset() + 66;	// set offset for serialization
   ...
   protected char[] checkWcdliNextCycDtIndConstraints(char[] value) {
	 ...
	 */ 
     ...
     int localWcdloStartDateIsoCounter = -1;
     public boolean isWcdloStartDateIsoModified() {
         int sharedCounter = shareString.getSerializedField().getModifiedCounter(); 
         boolean hasModified = localWcdloStartDateIsoCounter != sharedCounter;
         localWcdloStartDateIsoCounter = sharedCounter; return hasModified; 
     }     
	 ...
	 */
	public char[]  getWcdloStartDateIsoString() {
	     return getCharArray(beginWcdloStartDateIso,WCDLO_START_DATE_ISO_LEN);
	}
	 ...
	 *	@return true if numeric value is stored in the string
	 */
	public boolean wcdloStartDateIsoIsNumeric() {
	    return isNumeric(beginWcdloStartDateIso
	                    ,beginWcdloStartDateIso + WCDLO_START_DATE_ISO_LEN
	                    ,false/*Signed*/,true/*isSign trailing*/,false/*isSignStoredSeparately*/);
	}

  
   protected  static final int WCDLO_START_DATE_ISO_LEN = 8;
  	/**
	 * serializeWcdloStartDateIso
	 */
	protected void serializeWcdloStartDateIso(long wcdloStartDateIso) {
		 putNumber(beginWcdloStartDateIso,wcdloStartDateIso,WCDLO_START_DATE_ISO_LEN,false/*isSigned?*/,true/*signTrailing?*/,false/*storeSignSeparate?*/); 
		 localWcdloStartDateIsoCounter = shareString.getSerializedField().getModifiedCounter();

    }
    /**
	 * serializeWcdloStartDateIso
	 */
   	protected  long serializeWcdloStartDateIso(char[] value) {
	    long  wcdloStartDateIso;
	    if(value.length >0 && value.length!= 8)
            value = new String(value).trim().toCharArray();
	    if (value.length < 8) value = pad(8, value, ' ', LEFT_PAD);
	    else if (value.length > 8) value = substring(value,0,8);
	    /*  String can consists of digit or a non digit characters, in case of non digit characters, mimic COBOL Behavior and take only the last 4 bits per char and convert that to a number */
	    wcdloStartDateIso = convertString2Number(value,false/*isSigned?*/,true/*signTrailing?*/,false/*storeSignSeparate?*/);
		replaceValue(
		       padNumber(8,value,false/*isSigned?*/)
		       ,beginWcdloStartDateIso
		       ,8
		      );
		 localWcdloStartDateIsoCounter = shareString.getSerializedField().getModifiedCounter();
		return  wcdloStartDateIso;
   ...
   protected long checkWcdloStartDateIsoMaxLimit(long number) {

        return checkMaxLimit(number , MAX_100M/*limit*/  , false/*isSigned*/);
   }
    /**
	 *	refreshWcdloStartDateIso is used to refresh the latest value of a variable from the Serialized String
	 *  the most common reason to serialize an Object as a string in to write to a file. There can be several other reasons for serialization as well
	 */ 
   	public long refreshWcdloStartDateIso() throws CFException {
   	try {	 
			return (
			          getLongNumber(
			                  beginWcdloStartDateIso
			                 ,WCDLO_START_DATE_ISO_LEN
			                 ,false/*isSigned*/,true/*isSignTrailing*/,false/*isSignStoredSeparate*/)              			                 
			          ); 
	} catch(Exception ex) {
    	throw getSoc7ABend("wcdloStartDateIso", beginWcdloStartDateIso,WCDLO_START_DATE_ISO_LEN);
    }
   	}
     int localWcdloEndDateIsoCounter = -1;
     public boolean isWcdloEndDateIsoModified() {
         int sharedCounter = shareString.getSerializedField().getModifiedCounter(); 
         boolean hasModified = localWcdloEndDateIsoCounter != sharedCounter;
         localWcdloEndDateIsoCounter = sharedCounter; return hasModified; 
     }     
	 ...
	 */
	public char[]  getWcdloEndDateIsoString() {
	     return getCharArray(beginWcdloEndDateIso,WCDLO_END_DATE_ISO_LEN);
	}
	 ...
	 *	@return true if numeric value is stored in the string
	 */
	public boolean wcdloEndDateIsoIsNumeric() {
	    return isNumeric(beginWcdloEndDateIso
	                    ,beginWcdloEndDateIso + WCDLO_END_DATE_ISO_LEN
	                    ,false/*Signed*/,true/*isSign trailing*/,false/*isSignStoredSeparately*/);
	}

  
   protected  static final int WCDLO_END_DATE_ISO_LEN = 8;
  	/**
	 * serializeWcdloEndDateIso
	 */
	protected void serializeWcdloEndDateIso(long wcdloEndDateIso) {
		 putNumber(beginWcdloEndDateIso,wcdloEndDateIso,WCDLO_END_DATE_ISO_LEN,false/*isSigned?*/,true/*signTrailing?*/,false/*storeSignSeparate?*/); 
		 localWcdloEndDateIsoCounter = shareString.getSerializedField().getModifiedCounter();

    }
    /**
	 * serializeWcdloEndDateIso
	 */
   	protected  long serializeWcdloEndDateIso(char[] value) {
	    long  wcdloEndDateIso;
	    if(value.length >0 && value.length!= 8)
            value = new String(value).trim().toCharArray();
	    if (value.length < 8) value = pad(8, value, ' ', LEFT_PAD);
	    else if (value.length > 8) value = substring(value,0,8);
	    /*  String can consists of digit or a non digit characters, in case of non digit characters, mimic COBOL Behavior and take only the last 4 bits per char and convert that to a number */
	    wcdloEndDateIso = convertString2Number(value,false/*isSigned?*/,true/*signTrailing?*/,false/*storeSignSeparate?*/);
		replaceValue(
		       padNumber(8,value,false/*isSigned?*/)
		       ,beginWcdloEndDateIso
		       ,8
		      );
		 localWcdloEndDateIsoCounter = shareString.getSerializedField().getModifiedCounter();
		return  wcdloEndDateIso;
   ...
   protected long checkWcdloEndDateIsoMaxLimit(long number) {

        return checkMaxLimit(number , MAX_100M/*limit*/  , false/*isSigned*/);
   }
    /**
	 *	refreshWcdloEndDateIso is used to refresh the latest value of a variable from the Serialized String
	 *  the most common reason to serialize an Object as a string in to write to a file. There can be several other reasons for serialization as well
	 */ 
   	public long refreshWcdloEndDateIso() throws CFException {
   	try {	 
			return (
			          getLongNumber(
			                  beginWcdloEndDateIso
			                 ,WCDLO_END_DATE_ISO_LEN
			                 ,false/*isSigned*/,true/*isSignTrailing*/,false/*isSignStoredSeparate*/)              			                 
			          ); 
	} catch(Exception ex) {
    	throw getSoc7ABend("wcdloEndDateIso", beginWcdloEndDateIso,WCDLO_END_DATE_ISO_LEN);
    }
   	}
