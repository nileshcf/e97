...
/**
 * The Tmsmmcpy application can be started in three different ways
 * 1. Via mainframe JCL
	...
	public static void main(String[] args) {
		...
		if (StringUtils.hasText(appType) && WebApplicationType.NONE.equals(WebApplicationType.valueOf(appType.trim().toUpperCase())))
            ...
            GlobalExecutorCtx.load(CFApplication.class, context);
		 		...
		 		Tmsmmcpy tmsmmcpy = (Tmsmmcpy) context.getBean("tmsmmcpy");
        	    tmsmmcpy.process(new GlobalExecutorCtx().getContext(TmsmmcpyCtx.class)); /*  Tmsmmcpy requires no input parameters  */
