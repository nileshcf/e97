...
/**
 * The Dispif application can be started in three different ways
 * 1. Via mainframe JCL
	...
	public static void main(String[] args) {
		...
		if (StringUtils.hasText(appType) && WebApplicationType.NONE.equals(WebApplicationType.valueOf(appType.trim().toUpperCase())))
            ...
            GlobalExecutorCtx.load(CFApplication.class, context);
		 		...
		 		Dispif dispif = (Dispif) context.getBean("dispif");
        	    dispif.process(new GlobalExecutorCtx().getContext(DispifCtx.class)); /*  Dispif requires no input parameters  */
