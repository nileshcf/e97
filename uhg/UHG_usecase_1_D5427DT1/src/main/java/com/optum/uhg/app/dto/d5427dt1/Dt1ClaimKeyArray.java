...
public class Dt1ClaimKeyArray extends Dt1ClaimKeyArraySerialized {
  ...
  /**
   * Returns the value of dt1ClaimKeyEntry Corresponding COBOL Variable is DT1-CLAIM-KEY-ENTRY
   *
  ...
  public Dt1ClaimKeyEntry getDt1ClaimKeyEntry(int index) {
    if (index < 0) {
      ...
      logger.trace(
          "dt1ClaimKeyEntry - Array index exceeded max Size {}, resetting it to max allowed",
          DT_1_CLAIM_KEY_ENTRY_SIZE);
    }
    if (index >= dt1ClaimKeyEntry.size()) {
      ...
      dt1ClaimKeyEntry.set(
          index,
          new Dt1ClaimKeyEntry(
              this,
              beginDt1ClaimKeyEntry + index * Dt1ClaimKeyEntry.getDt1ClaimKeyEntryFieldLength()));
    }
  ...
  /**
   * Update Dt1ClaimKeyEntry at index with the passed value Corresponding COBOL Variable is
   * DT1-CLAIM-KEY-ENTRY
   *
   * @param index
   * @param value
