...
public class Sqlda extends SqldaSerialized {
  ...
  public Sqlda() {
    ...
    for (int arrayIndex = 0; arrayIndex < SQLVAR_SIZE; arrayIndex++) {
      sqlvar.add(new Sqlvar(this, beginSqlvar + arrayIndex * Sqlvar.getSqlvarFieldLength()));
    }
    /*  end of offset */
  }
  ...
  /**
   * set variable sqldaid Corresponding COBOL Variable is SQLDAID
   *
   * @param value
   */
  ...
  /**
   * Update Sqldaid with a char[] from an offset and length
   *
   * @param value
   */
  ...
  /**
   * Update Sqldaid with a char[] from an offset and length to an offset and length
   *
   * @param value
   */
  public void setSqldaid(
  ...
  /**
   * Update Sqldaid with another Field from an offset and length
   *
   * @param value
   */
  ...
  /**
   * Update Sqldaid with another Field from an offset and length to an offset and length
   *
   * @param value
   */
  public void setSqldaid(
  ...
  /**
   * Update Sqldabc with the passed value Corresponding COBOL Variable is SQLDABC
   *
   * @param number
   */
  ...
  /**
   * Update Sqln with the passed value Corresponding COBOL Variable is SQLN
   *
   * @param number
   */
  ...
  /**
   * Update Sqld with the passed value Corresponding COBOL Variable is SQLD
   *
   * @param number
   */
  ...
  /**
   * Returns the value of sqlvar Corresponding COBOL Variable is SQLVAR
   *
   * @return sqlvar
   */
  ...
  public Sqlvar getSqlvar(int index) {
    if (index < 0) {
      ...
      logger.trace(
          "sqlvar - Array index exceeded max Size {}, resetting it to max allowed", SQLVAR_SIZE);
    }
    if (index >= sqlvar.size()) {
      for (int fillIndex = sqlvar.size() - 1; fillIndex < index; fillIndex++) {
        sqlvar.add(null);
      }
      sqlvar.set(index, new Sqlvar(this, beginSqlvar + index * Sqlvar.getSqlvarFieldLength()));
    }
  ...
  /**
   * Update Sqlvar at index with the passed value Corresponding COBOL Variable is SQLVAR
   *
   * @param index
   * @param value
   */
