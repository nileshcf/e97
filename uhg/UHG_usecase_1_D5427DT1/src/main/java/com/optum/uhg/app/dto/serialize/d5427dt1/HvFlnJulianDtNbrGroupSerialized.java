...
/**
 * The class HvFlnJulianDtNbrGroupSerialized is used to define offsets in order to serialize in a
 * fixed String
 *
...
public class HvFlnJulianDtNbrGroupSerialized extends Field {
  ...
  /**
   * sets parent for this HvFlnJulianDtNbrGroupSerialized to the parent and set the serialize offset
   * to parameter begin
   *
   * @param parent
  ...
  public boolean isHvFlnJulianDtNbrModified() {
    ...
    localHvFlnJulianDtNbrCounter = sharedCounter;
    return hasModified;
  }
  ...
  protected void serializeHvFlnJulianDtNbr(char[] hvFlnJulianDtNbr) {
    ...
    arraycopy(
        hvFlnJulianDtNbr, 0, getStringValue(), beginHvFlnJulianDtNbr, HV_FLN_JULIAN_DT_NBR_LEN);
  ...
  /**
   * refreshHvFlnJulianDtNbr is used to refresh the latest value of a variable from the Serialized
   * String the most common reason to serialize an Object as a string in to write to a file. There
   * can be several other reasons for serialization as well
   */
  public char[] refreshHvFlnJulianDtNbr() {
    return (substring(
        getStringValue(), beginHvFlnJulianDtNbr, beginHvFlnJulianDtNbr + HV_FLN_JULIAN_DT_NBR_LEN));
  }
}
