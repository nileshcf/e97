...
public class StsDsmFormattedSqlErrMsg extends StsDsmFormattedSqlErrMsgSerialized {
  ...
  /**
   * Returns the value of stsDsmFrmtdSqlErrMsg Corresponding COBOL Variable is
   * STS-DSM-FRMTD-SQL-ERR-MSG
   *
  ...
  public char[] getStsDsmFrmtdSqlErrMsg(int index) {
    if (index < 0) {
      ...
      logger.trace(
          "stsDsmFrmtdSqlErrMsg - Array index exceeded max Size {}, resetting it to max allowed",
          STS_DSM_FRMTD_SQL_ERR_MSG_SIZE);
    }
    return getCharArray(
  ...
  /**
   * Update StsDsmFrmtdSqlErrMsg with the passed value at a given index Corresponding COBOL Variable
   * is STS-DSM-FRMTD-SQL-ERR-MSG
   *
   * @param index
   * @param value
   */
  ...
  /**
   * Update StsDsmFrmtdSqlErrMsg with the passed value at a given index and serializes if
   * setModified is true
   *
   * @param index
   * @param value
  ...
  private void setStsDsmFrmtdSqlErrMsg(int index, char[] value, boolean setModified) {
    if (index < 0) {
      logger.trace(
          "Array index was {} for setStsDsmFrmtdSqlErrMsg(int,String,boolean), reset it to 0",
          index);
      index = 0;
    }
  ...
  /**
   * initializes StsDsmFormattedSqlErrMsg String fields defaults to Spaces short/int/long defaults
