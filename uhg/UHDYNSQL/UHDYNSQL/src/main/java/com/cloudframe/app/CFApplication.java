...
/**
 * The Uhdynsql application can be started in three different ways
 * 1. Via mainframe JCL
	...
	public static void main(String[] args) {
		...
		if (StringUtils.hasText(appType) && WebApplicationType.NONE.equals(WebApplicationType.valueOf(appType.trim().toUpperCase())))
            ...
            GlobalExecutorCtx.load(CFApplication.class, context);
		 		...
		 		Uhdynsql uhdynsql = (Uhdynsql) context.getBean("uhdynsql");
        	    uhdynsql.process(new GlobalExecutorCtx().getContext(UhdynsqlCtx.class)); /*  Uhdynsql requires no input parameters  */
