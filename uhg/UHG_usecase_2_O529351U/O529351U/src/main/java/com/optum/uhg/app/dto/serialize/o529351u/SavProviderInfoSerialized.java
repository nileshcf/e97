...
/**
 * The class SavProviderInfoSerialized is used to define offsets in order to serialize in a fixed
 * String
 *
...
public class SavProviderInfoSerialized extends Field {
  ...
  /**
   * sets parent for this SavProviderInfoSerialized to the parent and set the serialize offset to
   * parameter begin
   *
   * @param parent
  ...
  public boolean isSavHAttendingPhysicianIdModified() {
    ...
    localSavHAttendingPhysicianIdCounter = sharedCounter;
    return hasModified;
  }
  ...
  /**
   * refreshSavHAttendingPhysicianId is used to refresh the latest value of a variable from the
  ...
  public char[] refreshSavHAttendingPhysicianId() {
    return (substring(
        getStringValue(),
        beginSavHAttendingPhysicianId,
        beginSavHAttendingPhysicianId + SAV_HATTENDING_PHYSICIAN_ID_LEN));
  }
}
