...
/**
 * The class SavProviderAreaSerialized is used to define offsets in order to serialize in a fixed
 * String
 *
...
public class SavProviderAreaSerialized extends Field {
  ...
  /**
   * sets parent for this SavProviderAreaSerialized to the parent and set the serialize offset to
   * parameter begin
   *
   * @param parent
  ...
  public boolean isSavProviderTinNumberModified() {
    ...
    localSavProviderTinNumberCounter = sharedCounter;
    return hasModified;
  }
  ...
  protected long serializeSavProviderTinNumber(char[] value) {
    ...
    replaceValue(padNumber(15, value, false /*isSigned?*/), beginSavProviderTinNumber, 15);
    localSavProviderTinNumberCounter = shareString.getSerializedField().getModifiedCounter();
    return savProviderTinNumber;
  }
  ...
  /**
   * refreshSavProviderTinNumber is used to refresh the latest value of a variable from the
  ...
  public long refreshSavProviderTinNumber() throws CFException {
    try {
      ...
      throw getSoc7ABend(
          "savProviderTinNumber", beginSavProviderTinNumber, SAV_PROVIDER_TIN_NUMBER_LEN);
    }
  }
}
