...
/**
 * The class SavDiagnosisCodePoaSerialized is used to define offsets in order to serialize in a
 * fixed String
 *
...
public class SavDiagnosisCodePoaSerialized extends Field {
  ...
  /**
   * sets parent for this SavDiagnosisCodePoaSerialized to the parent and set the serialize offset
   * to parameter begin
   *
   * @param parent
  ...
  protected void serializeSavDiagnosisCode(int index, char[] value) {
    replaceValue(
        value, (beginSavDiagnosisCode + index * SAV_DIAGNOSIS_CODE_LEN), SAV_DIAGNOSIS_CODE_LEN);
  }
