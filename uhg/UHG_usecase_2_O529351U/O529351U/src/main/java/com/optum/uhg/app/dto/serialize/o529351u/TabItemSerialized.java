...
/**
 * The class TabItemSerialized is used to define offsets in order to serialize in a fixed String
 *
...
public class TabItemSerialized extends Field {
  ...
  /**
   * sets parent for this TabItemSerialized to the parent and set the serialize offset to parameter
   * begin
   *
   * @param parent
  ...
  public boolean isNewTranslModified() {
    ...
    localNewTranslCounter = sharedCounter;
    return hasModified;
  }
  ...
  protected long serializeNewTransl(char[] value) {
    ...
    replaceValue(padNumber(6, value, false /*isSigned?*/), beginNewTransl, 6);
    localNewTranslCounter = shareString.getSerializedField().getModifiedCounter();
    return newTransl;
  }
