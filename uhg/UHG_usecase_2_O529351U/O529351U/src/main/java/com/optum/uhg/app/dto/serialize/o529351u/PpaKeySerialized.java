...
/**
 * The class PpaKeySerialized is used to define offsets in order to serialize in a fixed String
 *
...
public class PpaKeySerialized extends Field {
  ...
  /**
   * sets parent for this PpaKeySerialized to the parent and set the serialize offset to parameter
   * begin
   *
   * @param parent
  ...
  public boolean isPpaProvTinSuffModified() {
    ...
    localPpaProvTinSuffCounter = sharedCounter;
    return hasModified;
  }
  ...
  public boolean ppaProvTinSuffIsNumeric() {
    return decimalIsNumeric(
        beginPpaProvTinSuff,
        PPA_PROV_TIN_SUFF_LEN); // check Packed Decimal field to see if it contains valid Number
  }
  ...
  /**
   * refreshPpaProvTinSuff is used to refresh the latest value of a variable from the Serialized
