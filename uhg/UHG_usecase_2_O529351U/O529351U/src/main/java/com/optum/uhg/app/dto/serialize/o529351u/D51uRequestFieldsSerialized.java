...
/**
 * The class D51uRequestFieldsSerialized is used to define offsets in order to serialize in a fixed
 * String
 *
...
public class D51uRequestFieldsSerialized extends Field {
  ...
  /**
   * sets parent for this D51uRequestFieldsSerialized to the parent and set the serialize offset to
   * parameter begin
   *
   * @param parent
  ...
  public boolean isD51uReqCallerTypeModified() {
    ...
    localD51uReqCallerTypeCounter = sharedCounter;
    return hasModified;
  }
  ...
  protected void serializeD51uReqCallerType(char[] d51uReqCallerType) {
    ...
    arraycopy(
        d51uReqCallerType, 0, getStringValue(), beginD51uReqCallerType, D_51U_REQ_CALLER_TYPE_LEN);
  ...
  /**
   * refreshD51uReqCallerType is used to refresh the latest value of a variable from the Serialized
  ...
  public char[] refreshD51uReqCallerType() {
    return (substring(
        getStringValue(),
        beginD51uReqCallerType,
        beginD51uReqCallerType + D_51U_REQ_CALLER_TYPE_LEN));
  }
  ...
  public boolean isD51uReqCallingProgramModified() {
    ...
    localD51uReqCallingProgramCounter = sharedCounter;
    return hasModified;
  }
  ...
  protected void serializeD51uReqCallingProgram(char[] d51uReqCallingProgram) {
    shareString.getSerializedField().incrementCounter();
    arraycopy(
        d51uReqCallingProgram,
        0,
        getStringValue(),
        beginD51uReqCallingProgram,
        D_51U_REQ_CALLING_PROGRAM_LEN);
  ...
  /**
   * refreshD51uReqCallingProgram is used to refresh the latest value of a variable from the
  ...
  public char[] refreshD51uReqCallingProgram() {
    return (substring(
        getStringValue(),
        beginD51uReqCallingProgram,
        beginD51uReqCallingProgram + D_51U_REQ_CALLING_PROGRAM_LEN));
  }
  ...
  public boolean isD51uReqFuncCdModified() {
    ...
    localD51uReqFuncCdCounter = sharedCounter;
    return hasModified;
  }
  ...
  protected int serializeD51uReqFuncCd(char[] value) {
    ...
    replaceValue(padNumber(2, value, false /*isSigned?*/), beginD51uReqFuncCd, 2);
    localD51uReqFuncCdCounter = shareString.getSerializedField().getModifiedCounter();
    return d51uReqFuncCd;
  }
  ...
  /**
   * refreshD51uReqFuncCd is used to refresh the latest value of a variable from the Serialized
   * String the most common reason to serialize an Object as a string in to write to a file. There
   * can be several other reasons for serialization as well
   */
  ...
  public boolean isD51uReqTypeModified() {
    ...
    localD51uReqTypeCounter = sharedCounter;
    return hasModified;
  }
