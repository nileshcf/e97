...
public class D529351uRepositoryImpl implements D529351uRepository {
  ...
  /**
   * ***** DECLARE CSPLNV CURSOR ***** WITH ROWSET POSITIONING FOR ***** SELECT ***** A.SPI_TBL_ID
   * ***** ,B.POS_TIER_TYP_CD ***** ,B.PRI_DIAG_IND ***** ,B.MJR_DIAG_ELIG_IND ***** ***** FROM
   * FE_SPI_PLN_VAR A ***** ***** INNER JOIN FE_SPI_DESC_HDR B ***** ON B.SPI_TBL_ID = A.SPI_TBL_ID
   * AND ***** B.SPI_STS_CD = 'A' AND ***** B.STS_CD = 'A' ***** ***** WHERE A.POL_NBR =
   * :PLNV-POL-NBR ***** AND A.PLN_NBR = :PLNV-PLN-NBR ***** AND A.EFF_DT <= :WS-DB2-REQ-DOS-DATE
   * ***** AND A.CANC_DT >= :WS-DB2-REQ-DOS-DATE ***** AND A.STS_CD = 'A' ***** ***** ***** ORDER BY
   * ***** A.SPI_TBL_ID ASC, ***** A.EFF_DT DESC ***** ***** FOR FETCH ONLY *****
   */
  public ResultSet openCsplnvD529351u(DclfeSpiPlnVar dclfeSpiPlnVar, WorkArea workArea, Sqlca sqlca)
    ...
    try {
      ...
      csplnvResultSet =
          db2Base
              .startQuery(11 /* SQL Sequence*/, CONSTANTS.LITERAL_D529351U)
              .withSql(SQLS.SQL_893291460)
              .withInputs(
                  dclfeSpiPlnVar.getPlnvPolNbr(),
                  dclfeSpiPlnVar.getPlnvPlnNbr(),
                  workArea.getDb2ReqDosDate(),
                  workArea.getDb2ReqDosDate())
  ...
  /**
   * ***** FETCH NEXT ROWSET FROM CSPLNV ***** FOR 100 ROWS ***** INTO ***** :HV01-SPI-TBL-ID *****
   * ,:HV14-POS-TIER-TYP-CD ***** ,:HV0A-PRI-DIAG-IND ***** ,:HV15-MJR-DIAG-ELIG-IND
   */
    ...
    try {
      ...
      while (hasResults && ftchRowCnt < 100) {
        ...
        if (hasResults) {
          ...
          if (resultObj != null) {
            hvHostVariablesCsplnv.setHv14PosTierTypCd(
                ftchRowCnt, resultObj.toString().toCharArray());
          } else {
            sqlca.setSqlcode(-305);
          }
          ...
          if (resultObj != null) {
            hvHostVariablesCsplnv.setHv15MjrDiagEligInd(
                ftchRowCnt, resultObj.toString().toCharArray());
          } else {
            sqlca.setSqlcode(-305);
          }
  ...
  /**
   * ***** DECLARE CSFRTO CURSOR ***** WITH ROWSET POSITIONING FOR ***** SELECT *****
   * DERIV_PL_OF_SVC_CD ***** ,DERIV_SVC_CD ***** ,DERIV_CAUS_CD ***** ,CLM_FRM_SVC_CD *****
   * ,CLM_TO_SVC_CD ***** ,SVC_TYP_CD ***** FROM ***** FE_SPI_RULE_FR_TO_SVC ***** WHERE SPI_TBL_ID
   * = :FRTO-SPI-TBL-ID ***** AND STS_CD = 'A' ***** FOR FETCH ONLY
   */
  public ResultSet openCsfrtoD529351u(DclfeSpiRuleFrToSvc dclfeSpiRuleFrToSvc, Sqlca sqlca)
    ...
    try {
      ...
      csfrtoResultSet =
          db2Base
              .startQuery(12 /* SQL Sequence*/, CONSTANTS.LITERAL_D529351U)
  ...
  /**
   * ***** FETCH NEXT ROWSET FROM CSFRTO ***** FOR 100 ROWS ***** INTO ***** :HV08-SVC-DRV-POS *****
   * ,:HV09-SVC-DRV-SVC ***** ,:HV10-SVC-DRV-CAU ***** ,:HV11-CLM-FRM-SVC-CD *****
   * ,:HV12-CLM-TO-SVC-CD ***** ,:HV13-SVC-TYP-CD
   */
    ...
    try {
      ...
      while (hasResults && ftchRowCnt < 100) {
        ...
        if (hasResults) {
          ...
          if (resultObj != null) {
            hvHostVariablesCsfrto.setHv11ClmFrmSvcCd(
                ftchRowCnt, resultObj.toString().toCharArray());
          } else {
            sqlca.setSqlcode(-305);
          }
  ...
  /**
   * ***** DECLARE CSPTYP CURSOR FOR ***** SELECT ***** PROV_TYP ***** FROM FE_SPI_RULE_PROV_TYP
   * ***** WHERE SPI_TBL_ID = :PTYP-SPI-TBL-ID ***** AND STS_CD = 'A' ***** AND DERIV_PL_OF_SVC_CD =
   * :PTYP-DERIV-PL-OF-SVC-CD ***** AND DERIV_SVC_CD = :PTYP-DERIV-SVC-CD ***** AND DERIV_CAUS_CD =
   * :PTYP-DERIV-CAUS-CD ***** ***** FOR FETCH ONLY *****
   */
  public ResultSet openCsptypD529351u(DclfeSpiRuleProvTyp dclfeSpiRuleProvTyp, Sqlca sqlca)
    ...
    try {
      ...
      csptypResultSet =
          db2Base
              .startQuery(13 /* SQL Sequence*/, CONSTANTS.LITERAL_D529351U)
              .withSql(SQLS.SQL_1935646759)
              .withInputs(
                  dclfeSpiRuleProvTyp.getPtypSpiTblId(),
                  dclfeSpiRuleProvTyp.getPtypDerivPlOfSvcCd(),
                  dclfeSpiRuleProvTyp.getPtypDerivSvcCd(),
                  dclfeSpiRuleProvTyp.getPtypDerivCausCd())
              .withDataTypes(Types.CHAR, Types.CHAR, Types.CHAR, Types.CHAR)
              .queryForList();
    } catch (SQLException e) {
      fillSqlcaObject(sqlca, e);
  ...
  /** ***** FETCH CSPTYP ***** INTO ***** :PTYP-PROV-TYP */
  ...
  /**
   * ***** DECLARE CSICD CURSOR ***** WITH ROWSET POSITIONING FOR ***** SELECT *****
   * DERIV_PL_OF_SVC_CD ***** ,DERIV_SVC_CD ***** ,DERIV_CAUS_CD ***** ,CLM_ICD_IND *****
   * ,CLM_ICD_FROM_CD ***** ,CLM_ICD_TO_CD ***** FROM FE_SPI_RULE_ICD ***** WHERE SPI_TBL_ID =
   * :RICD-SPI-TBL-ID ***** AND STS_CD = 'A' ***** AND DERIV_PL_OF_SVC_CD = :RICD-DERIV-PL-OF-SVC-CD
   * ***** AND DERIV_SVC_CD = :RICD-DERIV-SVC-CD ***** AND DERIV_CAUS_CD = :RICD-DERIV-CAUS-CD *****
   * AND CLM_ICD_IND = :WS-LST-CLM-ICD-IND ***** AND ((CLM_ICD_FROM_CD <= :TEST-DIAG-01 AND *****
   * CLM_ICD_TO_CD >= :TEST-DIAG-01) OR ***** (CLM_ICD_FROM_CD <= :TEST-DIAG-02 AND *****
   * CLM_ICD_TO_CD >= :TEST-DIAG-02) OR ***** (CLM_ICD_FROM_CD <= :TEST-DIAG-03 AND *****
   * CLM_ICD_TO_CD >= :TEST-DIAG-03) OR ***** (CLM_ICD_FROM_CD <= :TEST-DIAG-04 AND *****
   * CLM_ICD_TO_CD >= :TEST-DIAG-04) OR ***** (CLM_ICD_FROM_CD <= :TEST-DIAG-05 AND *****
   * CLM_ICD_TO_CD >= :TEST-DIAG-05) OR ***** (CLM_ICD_FROM_CD <= :TEST-DIAG-06 AND *****
   * CLM_ICD_TO_CD >= :TEST-DIAG-06) OR ***** (CLM_ICD_FROM_CD <= :TEST-DIAG-07 AND *****
   * CLM_ICD_TO_CD >= :TEST-DIAG-07) OR ***** (CLM_ICD_FROM_CD <= :TEST-DIAG-08 AND *****
   * CLM_ICD_TO_CD >= :TEST-DIAG-08) OR ***** (CLM_ICD_FROM_CD <= :TEST-DIAG-09 AND *****
   * CLM_ICD_TO_CD >= :TEST-DIAG-09) OR ***** (CLM_ICD_FROM_CD <= :TEST-DIAG-10 AND *****
   * CLM_ICD_TO_CD >= :TEST-DIAG-10) OR ***** (CLM_ICD_FROM_CD <= :TEST-DIAG-11 AND *****
   * CLM_ICD_TO_CD >= :TEST-DIAG-11) OR ***** (CLM_ICD_FROM_CD <= :TEST-DIAG-12 AND *****
   * CLM_ICD_TO_CD >= :TEST-DIAG-12) OR ***** (CLM_ICD_FROM_CD <= :TEST-DIAG-13 AND *****
   * CLM_ICD_TO_CD >= :TEST-DIAG-13) OR ***** (CLM_ICD_FROM_CD <= :TEST-DIAG-14 AND *****
   * CLM_ICD_TO_CD >= :TEST-DIAG-14) OR ***** (CLM_ICD_FROM_CD <= :TEST-DIAG-15 AND *****
   * CLM_ICD_TO_CD >= :TEST-DIAG-15) OR ***** (CLM_ICD_FROM_CD <= :TEST-DIAG-16 AND *****
   * CLM_ICD_TO_CD >= :TEST-DIAG-16) OR ***** (CLM_ICD_FROM_CD <= :TEST-DIAG-17 AND *****
   * CLM_ICD_TO_CD >= :TEST-DIAG-17) OR ***** (CLM_ICD_FROM_CD <= :TEST-DIAG-18 AND *****
   * CLM_ICD_TO_CD >= :TEST-DIAG-18) OR ***** (CLM_ICD_FROM_CD <= :TEST-DIAG-19 AND *****
   * CLM_ICD_TO_CD >= :TEST-DIAG-19) OR ***** (CLM_ICD_FROM_CD <= :TEST-DIAG-20 AND *****
   * CLM_ICD_TO_CD >= :TEST-DIAG-20) OR ***** (CLM_ICD_FROM_CD <= :TEST-DIAG-21 AND *****
   * CLM_ICD_TO_CD >= :TEST-DIAG-21) OR ***** (CLM_ICD_FROM_CD <= :TEST-DIAG-22 AND *****
   * CLM_ICD_TO_CD >= :TEST-DIAG-22) OR ***** (CLM_ICD_FROM_CD <= :TEST-DIAG-23 AND *****
   * CLM_ICD_TO_CD >= :TEST-DIAG-23) OR ***** (CLM_ICD_FROM_CD <= :TEST-DIAG-24 AND *****
   * CLM_ICD_TO_CD >= :TEST-DIAG-24) OR ***** (CLM_ICD_FROM_CD <= :TEST-DIAG-25 AND *****
   * CLM_ICD_TO_CD >= :TEST-DIAG-25) OR ***** CLM_ICD_FROM_CD = :TEST-DIAG-01 OR *****
   * CLM_ICD_FROM_CD = :TEST-DIAG-02 OR ***** CLM_ICD_FROM_CD = :TEST-DIAG-03 OR *****
   * CLM_ICD_FROM_CD = :TEST-DIAG-04 OR ***** CLM_ICD_FROM_CD = :TEST-DIAG-05 OR *****
   * CLM_ICD_FROM_CD = :TEST-DIAG-06 OR ***** CLM_ICD_FROM_CD = :TEST-DIAG-07 OR *****
   * CLM_ICD_FROM_CD = :TEST-DIAG-08 OR ***** CLM_ICD_FROM_CD = :TEST-DIAG-09 OR *****
   * CLM_ICD_FROM_CD = :TEST-DIAG-10 OR ***** CLM_ICD_FROM_CD = :TEST-DIAG-11 OR *****
   * CLM_ICD_FROM_CD = :TEST-DIAG-12 OR ***** CLM_ICD_FROM_CD = :TEST-DIAG-13 OR *****
   * CLM_ICD_FROM_CD = :TEST-DIAG-14 OR ***** CLM_ICD_FROM_CD = :TEST-DIAG-15 OR *****
   * CLM_ICD_FROM_CD = :TEST-DIAG-16 OR ***** CLM_ICD_FROM_CD = :TEST-DIAG-17 OR *****
   * CLM_ICD_FROM_CD = :TEST-DIAG-18 OR ***** CLM_ICD_FROM_CD = :TEST-DIAG-19 OR *****
   * CLM_ICD_FROM_CD = :TEST-DIAG-20 OR ***** CLM_ICD_FROM_CD = :TEST-DIAG-21 OR *****
   * CLM_ICD_FROM_CD = :TEST-DIAG-22 OR ***** CLM_ICD_FROM_CD = :TEST-DIAG-23 OR *****
   * CLM_ICD_FROM_CD = :TEST-DIAG-24 OR ***** CLM_ICD_FROM_CD = :TEST-DIAG-25) ***** ***** ORDER BY
   * CLM_ICD_FROM_CD, CLM_ICD_TO_CD ***** ***** FOR FETCH ONLY *****
   */
  public ResultSet openCsicdD529351u(
    ...
    try {
      ...
      csicdResultSet =
          db2Base
              .startQuery(14 /* SQL Sequence*/, CONSTANTS.LITERAL_D529351U)
              .withSql(SQLS.SQL_836373005)
              .withInputs(
                  dclfeSpiRuleIcd.getRicdSpiTblId(),
                  dclfeSpiRuleIcd.getRicdDerivPlOfSvcCd(),
                  dclfeSpiRuleIcd.getRicdDerivSvcCd(),
                  dclfeSpiRuleIcd.getRicdDerivCausCd(),
                  lstIcdKey.getLstClmIcdInd(),
                  testVariables.getTestDiagArea().getTestDiag01(),
                  testVariables.getTestDiagArea().getTestDiag01(),
                  testVariables.getTestDiagArea().getTestDiag02(),
                  testVariables.getTestDiagArea().getTestDiag02(),
                  testVariables.getTestDiagArea().getTestDiag03(),
                  testVariables.getTestDiagArea().getTestDiag03(),
                  testVariables.getTestDiagArea().getTestDiag04(),
                  testVariables.getTestDiagArea().getTestDiag04(),
                  testVariables.getTestDiagArea().getTestDiag05(),
                  testVariables.getTestDiagArea().getTestDiag05(),
                  testVariables.getTestDiagArea().getTestDiag06(),
                  testVariables.getTestDiagArea().getTestDiag06(),
                  testVariables.getTestDiagArea().getTestDiag07(),
                  testVariables.getTestDiagArea().getTestDiag07(),
                  testVariables.getTestDiagArea().getTestDiag08(),
                  testVariables.getTestDiagArea().getTestDiag08(),
                  testVariables.getTestDiagArea().getTestDiag09(),
                  testVariables.getTestDiagArea().getTestDiag09(),
                  testVariables.getTestDiagArea().getTestDiag10(),
                  testVariables.getTestDiagArea().getTestDiag10(),
                  testVariables.getTestDiagArea().getTestDiag11(),
                  testVariables.getTestDiagArea().getTestDiag11(),
                  testVariables.getTestDiagArea().getTestDiag12(),
                  testVariables.getTestDiagArea().getTestDiag12(),
                  testVariables.getTestDiagArea().getTestDiag13(),
                  testVariables.getTestDiagArea().getTestDiag13(),
                  testVariables.getTestDiagArea().getTestDiag14(),
                  testVariables.getTestDiagArea().getTestDiag14(),
                  testVariables.getTestDiagArea().getTestDiag15(),
                  testVariables.getTestDiagArea().getTestDiag15(),
                  testVariables.getTestDiagArea().getTestDiag16(),
                  testVariables.getTestDiagArea().getTestDiag16(),
                  testVariables.getTestDiagArea().getTestDiag17(),
                  testVariables.getTestDiagArea().getTestDiag17(),
                  testVariables.getTestDiagArea().getTestDiag18(),
                  testVariables.getTestDiagArea().getTestDiag18(),
                  testVariables.getTestDiagArea().getTestDiag19(),
                  testVariables.getTestDiagArea().getTestDiag19(),
                  testVariables.getTestDiagArea().getTestDiag20(),
                  testVariables.getTestDiagArea().getTestDiag20(),
                  testVariables.getTestDiagArea().getTestDiag21(),
                  testVariables.getTestDiagArea().getTestDiag21(),
                  testVariables.getTestDiagArea().getTestDiag22(),
                  testVariables.getTestDiagArea().getTestDiag22(),
                  testVariables.getTestDiagArea().getTestDiag23(),
                  testVariables.getTestDiagArea().getTestDiag23(),
                  testVariables.getTestDiagArea().getTestDiag24(),
                  testVariables.getTestDiagArea().getTestDiag24(),
                  testVariables.getTestDiagArea().getTestDiag25(),
                  testVariables.getTestDiagArea().getTestDiag25(),
                  testVariables.getTestDiagArea().getTestDiag01(),
                  testVariables.getTestDiagArea().getTestDiag02(),
                  testVariables.getTestDiagArea().getTestDiag03(),
  ...
  /**
   * ***** FETCH NEXT ROWSET FROM CSICD ***** FOR 9999 ROWS ***** INTO ***** :HV3A-ICD-DRV-POS *****
   * ,:HV3B-ICD-DRV-SVC ***** ,:HV3C-ICD-DRV-CAU ***** ,:HV3D-ICD-TYP-CD ***** ,:HV3E-CLM-FRM-ICD-CD
   * ***** ,:HV3F-CLM-TO-ICD-CD
   */
    ...
    try {
      ...
      while (hasResults && ftchRowCnt < 9999) {
        ...
        if (hasResults) {
          ...
          if (resultObj != null) {
            hvHostVariablesCsicdo.setHv3eClmFrmIcdCd(
                ftchRowCnt, resultObj.toString().toCharArray());
          } else {
            sqlca.setSqlcode(-305);
          }
  ...
  /**
   * ***** DECLARE CSFCND CURSOR FOR ***** SELECT ***** FROM_COND_POS ***** FROM
   * FE_SPI_RULE_FR_COND_POS ***** WHERE SPI_TBL_ID = :FCND-SPI-TBL-ID ***** AND STS_CD = 'A' *****
   * AND DERIV_PL_OF_SVC_CD = :FCND-DERIV-PL-OF-SVC-CD ***** AND DERIV_SVC_CD = :FCND-DERIV-SVC-CD
   * ***** AND DERIV_CAUS_CD = :FCND-DERIV-CAUS-CD ***** ***** FOR FETCH ONLY *****
   */
  public ResultSet openCsfcndD529351u(Sqlca sqlca, DclfeSpiRuleFrCondPos dclfeSpiRuleFrCondPos)
    ...
    try {
      ...
      csfcndResultSet =
          db2Base
              .startQuery(16 /* SQL Sequence*/, CONSTANTS.LITERAL_D529351U)
              .withSql(SQLS.SQL_443604161)
              .withInputs(
                  dclfeSpiRuleFrCondPos.getFcndSpiTblId(),
                  dclfeSpiRuleFrCondPos.getFcndDerivPlOfSvcCd(),
                  dclfeSpiRuleFrCondPos.getFcndDerivSvcCd(),
                  dclfeSpiRuleFrCondPos.getFcndDerivCausCd())
              .withDataTypes(Types.CHAR, Types.CHAR, Types.CHAR, Types.CHAR)
              .queryForList();
    } catch (SQLException e) {
      fillSqlcaObject(sqlca, e);
  ...
  /** ***** FETCH CSFCND ***** INTO ***** :FCND-FROM-COND-POS */
  ...
  /**
   * ***** SELECT ***** CLM_CAUS_1_CD ***** ,CLM_CAUS_2_CD ***** ,CLM_CAUS_3_CD ***** ,CLM_CAUS_4_CD
   * ***** ,CLM_CAUS_5_CD ***** ,CLM_CAUS_6_CD ***** ,CLM_CAUS_7_CD ***** ,CLM_CAUS_8_CD ***** INTO
   * ***** :CAUS-CLM-CAUS-1-CD ***** ,:CAUS-CLM-CAUS-2-CD ***** ,:CAUS-CLM-CAUS-3-CD *****
   * ,:CAUS-CLM-CAUS-4-CD ***** ,:CAUS-CLM-CAUS-5-CD ***** ,:CAUS-CLM-CAUS-6-CD *****
   * ,:CAUS-CLM-CAUS-7-CD ***** ,:CAUS-CLM-CAUS-8-CD ***** FROM FE_SPI_RULE_CAUS ***** WHERE
   * SPI_TBL_ID = :CAUS-SPI-TBL-ID ***** AND STS_CD = 'A' ***** AND DERIV_PL_OF_SVC_CD =
   * :CAUS-DERIV-PL-OF-SVC-CD ***** AND DERIV_SVC_CD = :CAUS-DERIV-SVC-CD ***** AND DERIV_CAUS_CD =
   * :CAUS-DERIV-CAUS-CD ***** FETCH FIRST 1 ROW ONLY *****
   */
  public void selectFeSpiRuleCaus(DclfeSpiRuleCaus dclfeSpiRuleCaus, Sqlca sqlca) throws Exception {
    try {
      ...
      ResultSet resultSet =
          db2Base
              .startQuery(33 /* SQL Sequence*/, CONSTANTS.LITERAL_D529351U)
              .withSql(SQLS.SQL_1320672265)
              .withInputs(
                  dclfeSpiRuleCaus.getCausSpiTblId(),
                  dclfeSpiRuleCaus.getCausDerivPlOfSvcCd(),
                  dclfeSpiRuleCaus.getCausDerivSvcCd(),
                  dclfeSpiRuleCaus.getCausDerivCausCd())
              .withDataTypes(Types.CHAR, Types.CHAR, Types.CHAR, Types.CHAR)
              .queryForList();
  ...
  /**
   * ***** SELECT ***** CLM_IPA_1_CD ***** ,CLM_IPA_2_CD ***** ,CLM_IPA_3_CD ***** INTO *****
   * :RIPA-CLM-IPA-1-CD ***** ,:RIPA-CLM-IPA-2-CD ***** ,:RIPA-CLM-IPA-3-CD ***** FROM
   * FE_SPI_RULE_IPA ***** WHERE SPI_TBL_ID = :RIPA-SPI-TBL-ID ***** AND STS_CD = 'A' ***** AND
   * DERIV_PL_OF_SVC_CD = :RIPA-DERIV-PL-OF-SVC-CD ***** AND DERIV_SVC_CD = :RIPA-DERIV-SVC-CD *****
   * AND DERIV_CAUS_CD = :RIPA-DERIV-CAUS-CD ***** FETCH FIRST 1 ROW ONLY *****
   */
  public void selectFeSpiRuleIpa(DclfeSpiRuleIpa dclfeSpiRuleIpa, Sqlca sqlca) throws Exception {
    try {
      ...
      ResultSet resultSet =
          db2Base
              .startQuery(34 /* SQL Sequence*/, CONSTANTS.LITERAL_D529351U)
              .withSql(SQLS.SQL_1233467357)
              .withInputs(
                  dclfeSpiRuleIpa.getRipaSpiTblId(),
                  dclfeSpiRuleIpa.getRipaDerivPlOfSvcCd(),
                  dclfeSpiRuleIpa.getRipaDerivSvcCd(),
                  dclfeSpiRuleIpa.getRipaDerivCausCd())
              .withDataTypes(Types.CHAR, Types.CHAR, Types.CHAR, Types.CHAR)
              .queryForList();
      // check sqlca.getSqlcode() or errors
      sqlca.setSqlcode(Db2Base.fillSQLCode(resultSet));
  ...
  /**
   * ***** DECLARE SVC-MAIN-CSR CURSOR ***** WITH ROWSET POSITIONING FOR ***** SELECT *****
   * DERIV_PL_OF_SVC_CD ***** ,DERIV_SVC_CD ***** ,DERIV_CAUS_CD ***** ***** ,POS_IND || *****
   * SVC_IND || ***** PROV_IND || ***** CAUS_IND || ***** IPA_IND || ***** GTN_IND || ***** ICD_IND
   * || ***** CPE_DESC_IND ***** ***** FROM ***** FE_SPI_DENORM ***** WHERE ***** SPI_TBL_ID =
   * :FRTO-SPI-TBL-ID ***** FOR FETCH ONLY ***** WITH UR
   */
  public ResultSet openSvcMainCsrD529351u(DclfeSpiRuleFrToSvc dclfeSpiRuleFrToSvc, Sqlca sqlca)
    ...
    try {
      ...
      svcMainCsrResultSet =
          db2Base
              .startQuery(10 /* SQL Sequence*/, CONSTANTS.LITERAL_D529351U)
  ...
  /**
   * ***** FETCH NEXT ROWSET FROM SVC-MAIN-CSR ***** FOR 100 ROWS ***** INTO ***** :HV04-KEY-DRV-POS
   * ***** , :HV05-KEY-DRV-SVC ***** , :HV06-KEY-DRV-CAU ***** , :HV07-RULE-SW
   */
      ...
      while (hasResults && ftchRowCnt < 100) {
        ...
        if (hasResults) {
          ...
          if (resultObjString != null) {
            hvHostVariablesSvcmain.setHv07RuleSw(
                ftchRowCnt, db2Base.formatISOTime(resultObjString));
          } else {
            sqlca.setSqlcode(-305);
          }
          sqlFetchCounter++;
  ...
  /**
   * ***** DECLARE COMB_CUR CURSOR ***** WITH ROWSET POSITIONING FOR ***** SELECT '1' REC_TYP_CD
   * ***** ,DERIV_PL_OF_SVC_CD ***** ,DERIV_SVC_CD ***** ,DERIV_CAUS_CD ***** ,CLM_CAUS_1_CD *****
   * ,CLM_CAUS_2_CD ***** ,CLM_CAUS_3_CD ***** ,CLM_CAUS_4_CD ***** ,CLM_CAUS_5_CD *****
   * ,CLM_CAUS_6_CD ***** ,CLM_CAUS_7_CD ***** ,CLM_CAUS_8_CD ***** ,CHAR(SPACE(3)) CLM_IPA_1_CD
   * ***** ,CHAR(SPACE(3)) CLM_IPA_2_CD ***** ,CHAR(SPACE(3)) CLM_IPA_3_CD ***** ,CHAR(SPACE(2))
   * PROV_TYP ***** ,CHAR(SPACE(2)) FROM_COND_POS ***** ,CHAR(SPACE(4)) GRP_TBL_1_NBR *****
   * ,CHAR(SPACE(4)) GRP_TBL_2_NBR ***** ,CHAR(SPACE(4)) GRP_TBL_3_NBR ***** ,CHAR(SPACE(4))
   * CPE_DESC_CD ***** FROM FE_SPI_RULE_CAUS ***** WHERE SPI_TBL_ID = :CAUS-SPI-TBL-ID ***** AND
   * STS_CD = 'A' ***** AND (CLM_CAUS_1_CD = :TEST-CAUS OR ***** CLM_CAUS_2_CD = :TEST-CAUS OR *****
   * CLM_CAUS_3_CD = :TEST-CAUS OR ***** CLM_CAUS_4_CD = :TEST-CAUS OR ***** CLM_CAUS_5_CD =
   * :TEST-CAUS OR ***** CLM_CAUS_6_CD = :TEST-CAUS OR ***** CLM_CAUS_7_CD = :TEST-CAUS OR *****
   * CLM_CAUS_8_CD = :TEST-CAUS) ***** UNION ALL ***** SELECT '2' REC_TYP_CD *****
   * ,DERIV_PL_OF_SVC_CD ***** ,DERIV_SVC_CD ***** ,DERIV_CAUS_CD ***** ,CHAR(SPACE(1))
   ...
   * ,CLM_IPA_1_CD ***** ,CLM_IPA_2_CD ***** ,CLM_IPA_3_CD ***** ,CHAR(SPACE(2)) PROV_TYP *****
   * ,CHAR(SPACE(2)) FROM_COND_POS ***** ,GRP_TBL_1_NBR ***** ,GRP_TBL_2_NBR ***** ,GRP_TBL_3_NBR
   * ***** ,CHAR(SPACE(4)) CPE_DESC_CD ***** FROM FE_SPI_RULE_IPA ***** WHERE SPI_TBL_ID =
   * :RIPA-SPI-TBL-ID ***** AND STS_CD = 'A' ***** UNION ALL ***** SELECT '3' REC_TYP_CD *****
   * ,DERIV_PL_OF_SVC_CD ***** ,DERIV_SVC_CD ***** ,DERIV_CAUS_CD ***** ,CHAR(SPACE(1))
   * CLM_CAUS_1_CD ***** ,CHAR(SPACE(1)) CLM_CAUS_2_CD ***** ,CHAR(SPACE(1)) CLM_CAUS_3_CD *****
   * ,CHAR(SPACE(1)) CLM_CAUS_4_CD ***** ,CHAR(SPACE(1)) CLM_CAUS_5_CD ***** ,CHAR(SPACE(1))
   * CLM_CAUS_6_CD ***** ,CHAR(SPACE(1)) CLM_CAUS_7_CD ***** ,CHAR(SPACE(1)) CLM_CAUS_8_CD *****
   * ,CHAR(SPACE(3)) CLM_IPA_1_CD ***** ,CHAR(SPACE(3)) CLM_IPA_2_CD ***** ,CHAR(SPACE(3))
   * CLM_IPA_3_CD ***** ,PROV_TYP ***** ,CHAR(SPACE(2)) FROM_COND_POS ***** ,CHAR(SPACE(4))
   ...
   * :PTYP-SPI-TBL-ID ***** AND STS_CD = 'A' ***** AND PROV_TYP = :TEST-PROV-TYP ***** ***** UNION
   * ALL ***** SELECT '4' REC_TYP_CD ***** ,DERIV_PL_OF_SVC_CD ***** ,DERIV_SVC_CD *****
   ...
   * CLM_IPA_2_CD ***** ,CHAR(SPACE(3)) CLM_IPA_3_CD ***** ,CHAR(SPACE(2)) PROV_TYP *****
   ...
   * FE_SPI_RULE_FR_COND_POS ***** WHERE SPI_TBL_ID = :FCND-SPI-TBL-ID ***** AND STS_CD = 'A' *****
   * AND FROM_COND_POS = :TEST-POS ***** ***** UNION ALL ***** SELECT '5' REC_TYP_CD *****
   * ,DERIV_PL_OF_SVC_CD ***** ,DERIV_SVC_CD ***** ,DERIV_CAUS_CD ***** ,CHAR(SPACE(1))
   * CLM_CAUS_1_CD ***** ,CHAR(SPACE(1)) CLM_CAUS_2_CD ***** ,CHAR(SPACE(1)) CLM_CAUS_3_CD *****
   * ,CHAR(SPACE(1)) CLM_CAUS_4_CD ***** ,CHAR(SPACE(1)) CLM_CAUS_5_CD ***** ,CHAR(SPACE(1))
   * CLM_CAUS_6_CD ***** ,CHAR(SPACE(1)) CLM_CAUS_7_CD ***** ,CHAR(SPACE(1)) CLM_CAUS_8_CD *****
   * ,CHAR(SPACE(3)) CLM_IPA_1_CD ***** ,CHAR(SPACE(3)) CLM_IPA_2_CD ***** ,CHAR(SPACE(3))
   * CLM_IPA_3_CD ***** ,CHAR(SPACE(2)) PROV_TYP ***** ,CHAR(SPACE(2)) FROM_COND_POS *****
   * ,CHAR(SPACE(4)) GRP_TBL_1_NBR ***** ,CHAR(SPACE(4)) GRP_TBL_2_NBR ***** ,CHAR(SPACE(4))
   * GRP_TBL_3_NBR ***** ,CPE_DESC_CD ***** FROM FE_SPI_RULE_CPE_FLG ***** WHERE SPI_TBL_ID =
   * :CPEFL-SPI-TBL-ID ***** AND STS_CD = 'A' ***** AND CPE_DESC_CD = :TEST-CPE-DESC-CD ***** *****
   * FOR FETCH ONLY
   */
  public ResultSet openCombCurD529351u(
    ...
    try {
      ...
      combCurResultSet =
          db2Base
              .startQuery(17 /* SQL Sequence*/, CONSTANTS.LITERAL_D529351U)
              .withSql(SQLS.SQL_1809880364)
              .withInputs(
                  dclfeSpiRuleCaus.getCausSpiTblId(),
                  testVariables.getTestCaus(),
                  testVariables.getTestCaus(),
                  testVariables.getTestCaus(),
                  testVariables.getTestCaus(),
                  testVariables.getTestCaus(),
                  ...
                  dclfeSpiRuleIpa.getRipaSpiTblId(),
                  dclfeSpiRuleProvTyp.getPtypSpiTblId(),
                  testVariables.getTestProvTyp(),
                  dclfeSpiRuleFrCondPos.getFcndSpiTblId(),
                  testVariables.getTestPos(),
                  dclfeSpiRuleCpeFlg.getCpeflSpiTblId(),
                  testVariables.getTestCpeDescCd())
              .withDataTypes(
                  Types.CHAR,
                  Types.CHAR,
                  Types.CHAR,
  ...
  /**
   * ***** FETCH NEXT ROWSET FROM COMB_CUR ***** FOR 100 ROWS ***** INTO ***** :HV2A-REC-TYP-CD,
   * ***** :HV2B-DERIV-PL-OF-SVC-CD, ***** :HV2C-DERIV-SVC-CD, ***** :HV2D-DERIV-CAUS-CD, *****
   * :HV2E-CLM-CAUS-1-CD, ***** :HV2F-CLM-CAUS-2-CD, ***** :HV2G-CLM-CAUS-3-CD, *****
   * :HV2H-CLM-CAUS-4-CD, ***** :HV2I-CLM-CAUS-5-CD, ***** :HV2J-CLM-CAUS-6-CD, *****
   * :HV2K-CLM-CAUS-7-CD, ***** :HV2L-CLM-CAUS-8-CD, ***** :HV2M-CLM-IPA-1-CD, *****
   * :HV2N-CLM-IPA-2-CD, ***** :HV2O-CLM-IPA-3-CD, ***** :HV2P-PROV-TYP, ***** :HV2Q-FROM-COND-POS,
   * ***** :HV2R-GRP-TBL-1-NBR, ***** :HV2S-GRP-TBL-2-NBR, ***** :HV2T-GRP-TBL-3-NBR, *****
   * :HV2U-CPE-DESC-CD
   */
    ...
    try {
      ...
      while (hasResults && ftchRowCnt < 100) {
        ...
        if (hasResults) {
          ...
          if (resultObj != null) {
            hvHostVariablesCombcr.setHv2bDerivPlOfSvcCd(
                ftchRowCnt, resultObj.toString().toCharArray());
          } else {
            sqlca.setSqlcode(-305);
          }
          ...
          if (resultObj != null) {
            hvHostVariablesCombcr.setHv2dDerivCausCd(
                ftchRowCnt, resultObj.toString().toCharArray());
          } else {
            sqlca.setSqlcode(-305);
          }
          ...
          if (resultObj != null) {
            hvHostVariablesCombcr.setHv2qFromCondPos(
                ftchRowCnt, resultObj.toString().toCharArray());
          } else {
            sqlca.setSqlcode(-305);
          }
  ...
  /**
   * ***** DECLARE CSICD_FIRST_9999_SPI CURSOR ***** WITH ROWSET POSITIONING FOR ***** SELECT *****
   * DERIV_PL_OF_SVC_CD ***** ,DERIV_SVC_CD ***** ,DERIV_CAUS_CD ***** ,CLM_ICD_IND *****
   * ,CLM_ICD_FROM_CD ***** ,CLM_ICD_TO_CD ***** FROM FE_SPI_RULE_ICD ***** WHERE (SPI_TBL_ID =
   * :RICD-SPI-TBL-ID ***** AND STS_CD = 'A' ***** AND CLM_ICD_IND = :WS-LST-CLM-ICD-IND) *****
   * ***** AND ((CLM_ICD_FROM_CD <= :TEST-DIAG-01 AND ***** CLM_ICD_TO_CD >= :TEST-DIAG-01) OR *****
   * (CLM_ICD_FROM_CD <= :TEST-DIAG-02 AND ***** CLM_ICD_TO_CD >= :TEST-DIAG-02) OR *****
   * (CLM_ICD_FROM_CD <= :TEST-DIAG-03 AND ***** CLM_ICD_TO_CD >= :TEST-DIAG-03) OR *****
   * (CLM_ICD_FROM_CD <= :TEST-DIAG-04 AND ***** CLM_ICD_TO_CD >= :TEST-DIAG-04) OR *****
   * (CLM_ICD_FROM_CD <= :TEST-DIAG-05 AND ***** CLM_ICD_TO_CD >= :TEST-DIAG-05) OR *****
   * (CLM_ICD_FROM_CD <= :TEST-DIAG-06 AND ***** CLM_ICD_TO_CD >= :TEST-DIAG-06) OR *****
   * (CLM_ICD_FROM_CD <= :TEST-DIAG-07 AND ***** CLM_ICD_TO_CD >= :TEST-DIAG-07) OR *****
   * (CLM_ICD_FROM_CD <= :TEST-DIAG-08 AND ***** CLM_ICD_TO_CD >= :TEST-DIAG-08) OR *****
   * (CLM_ICD_FROM_CD <= :TEST-DIAG-09 AND ***** CLM_ICD_TO_CD >= :TEST-DIAG-09) OR *****
   * (CLM_ICD_FROM_CD <= :TEST-DIAG-10 AND ***** CLM_ICD_TO_CD >= :TEST-DIAG-10) OR *****
   * (CLM_ICD_FROM_CD <= :TEST-DIAG-11 AND ***** CLM_ICD_TO_CD >= :TEST-DIAG-11) OR *****
   * (CLM_ICD_FROM_CD <= :TEST-DIAG-12 AND ***** CLM_ICD_TO_CD >= :TEST-DIAG-12) OR *****
   * (CLM_ICD_FROM_CD <= :TEST-DIAG-13 AND ***** CLM_ICD_TO_CD >= :TEST-DIAG-13) OR *****
   * (CLM_ICD_FROM_CD <= :TEST-DIAG-14 AND ***** CLM_ICD_TO_CD >= :TEST-DIAG-14) OR *****
   * (CLM_ICD_FROM_CD <= :TEST-DIAG-15 AND ***** CLM_ICD_TO_CD >= :TEST-DIAG-15) OR *****
   * (CLM_ICD_FROM_CD <= :TEST-DIAG-16 AND ***** CLM_ICD_TO_CD >= :TEST-DIAG-16) OR *****
   * (CLM_ICD_FROM_CD <= :TEST-DIAG-17 AND ***** CLM_ICD_TO_CD >= :TEST-DIAG-17) OR *****
   * (CLM_ICD_FROM_CD <= :TEST-DIAG-18 AND ***** CLM_ICD_TO_CD >= :TEST-DIAG-18) OR *****
   * (CLM_ICD_FROM_CD <= :TEST-DIAG-19 AND ***** CLM_ICD_TO_CD >= :TEST-DIAG-19) OR *****
   * (CLM_ICD_FROM_CD <= :TEST-DIAG-20 AND ***** CLM_ICD_TO_CD >= :TEST-DIAG-20) OR *****
   * (CLM_ICD_FROM_CD <= :TEST-DIAG-21 AND ***** CLM_ICD_TO_CD >= :TEST-DIAG-21) OR *****
   * (CLM_ICD_FROM_CD <= :TEST-DIAG-22 AND ***** CLM_ICD_TO_CD >= :TEST-DIAG-22) OR *****
   * (CLM_ICD_FROM_CD <= :TEST-DIAG-23 AND ***** CLM_ICD_TO_CD >= :TEST-DIAG-23) OR *****
   * (CLM_ICD_FROM_CD <= :TEST-DIAG-24 AND ***** CLM_ICD_TO_CD >= :TEST-DIAG-24) OR *****
   * (CLM_ICD_FROM_CD <= :TEST-DIAG-25 AND ***** CLM_ICD_TO_CD >= :TEST-DIAG-25) OR *****
   * CLM_ICD_FROM_CD = :TEST-DIAG-01 OR ***** CLM_ICD_FROM_CD = :TEST-DIAG-02 OR *****
   * CLM_ICD_FROM_CD = :TEST-DIAG-03 OR ***** CLM_ICD_FROM_CD = :TEST-DIAG-04 OR *****
   * CLM_ICD_FROM_CD = :TEST-DIAG-05 OR ***** CLM_ICD_FROM_CD = :TEST-DIAG-06 OR *****
   * CLM_ICD_FROM_CD = :TEST-DIAG-07 OR ***** CLM_ICD_FROM_CD = :TEST-DIAG-08 OR *****
   * CLM_ICD_FROM_CD = :TEST-DIAG-09 OR ***** CLM_ICD_FROM_CD = :TEST-DIAG-10 OR *****
   * CLM_ICD_FROM_CD = :TEST-DIAG-11 OR ***** CLM_ICD_FROM_CD = :TEST-DIAG-12 OR *****
   * CLM_ICD_FROM_CD = :TEST-DIAG-13 OR ***** CLM_ICD_FROM_CD = :TEST-DIAG-14 OR *****
   * CLM_ICD_FROM_CD = :TEST-DIAG-15 OR ***** CLM_ICD_FROM_CD = :TEST-DIAG-16 OR *****
   * CLM_ICD_FROM_CD = :TEST-DIAG-17 OR ***** CLM_ICD_FROM_CD = :TEST-DIAG-18 OR *****
   * CLM_ICD_FROM_CD = :TEST-DIAG-19 OR ***** CLM_ICD_FROM_CD = :TEST-DIAG-20 OR *****
   * CLM_ICD_FROM_CD = :TEST-DIAG-21 OR ***** CLM_ICD_FROM_CD = :TEST-DIAG-22 OR *****
   * CLM_ICD_FROM_CD = :TEST-DIAG-23 OR ***** CLM_ICD_FROM_CD = :TEST-DIAG-24 OR *****
   * CLM_ICD_FROM_CD = :TEST-DIAG-25) ***** ***** ORDER BY DERIV_PL_OF_SVC_CD, ***** DERIV_SVC_CD,
   * DERIV_CAUS_CD, CLM_ICD_FROM_CD, ***** CLM_ICD_TO_CD ***** ***** FOR FETCH ONLY *****
   */
  public ResultSet openCsicdFirst9999SpiD529351u(
    ...
    try {
      // OPEN CURSOR
      sqlca.setSqlcode(0);
      csicdFirst9999SpiResultSet =
          db2Base
              .startQuery(15 /* SQL Sequence*/, CONSTANTS.LITERAL_D529351U)
   ...
   * :HV3A-ICD-DRV-POS ***** ,:HV3B-ICD-DRV-SVC ***** ,:HV3C-ICD-DRV-CAU ***** ,:HV3D-ICD-TYP-CD
   * ***** ,:HV3E-CLM-FRM-ICD-CD ***** ,:HV3F-CLM-TO-ICD-CD
   */
