...
public class GenTransSpiTableR extends GenTransSpiTableRSerialized {
  ...
  /**
   * Returns the value of tableItem Corresponding COBOL Variable is TABLE-ITEM
   *
   * @return tableItem
   */
  ...
  public TableItem getTableItem(int index) {
    if (index < 0) {
      ...
      logger.trace(
          "tableItem - Array index exceeded max Size {}, resetting it to max allowed",
          TABLE_ITEM_SIZE);
    }
    if (index >= tableItem.size()) {
      ...
      tableItem.set(
          index, new TableItem(this, beginTableItem + index * TableItem.getTableItemFieldLength()));
    }
  ...
  /**
   * Update TableItem at index with the passed value Corresponding COBOL Variable is TABLE-ITEM
   *
   * @param index
   * @param value
   */
