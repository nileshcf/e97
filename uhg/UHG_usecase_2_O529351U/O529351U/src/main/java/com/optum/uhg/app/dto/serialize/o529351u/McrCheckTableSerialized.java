...
/**
 * The class McrCheckTableSerialized is used to define offsets in order to serialize in a fixed
 * String
 *
...
public class McrCheckTableSerialized extends Field {
  ...
  /**
   * sets parent for this McrCheckTableSerialized to the parent and set the serialize offset to
   * parameter begin
   *
   * @param parent
  ...
  public boolean isMcrLineNbrModified() {
    ...
    localMcrLineNbrCounter = sharedCounter;
    return hasModified;
  }
  ...
  protected int serializeMcrLineNbr(char[] value) {
    ...
    replaceValue(padNumber(1, value, false /*isSigned?*/), beginMcrLineNbr, 1);
    localMcrLineNbrCounter = shareString.getSerializedField().getModifiedCounter();
    return mcrLineNbr;
  }
  ...
  public boolean isMcrLineChargeModified() {
    ...
    localMcrLineChargeCounter = sharedCounter;
    return hasModified;
  }
  ...
  protected BigDecimal serializeMcrLineCharge(char[] value) throws CFException {
    ...
    replaceValue(padNumber(11, value, true /*isSigned?*/), beginMcrLineCharge, 11);
    localMcrLineChargeCounter = shareString.getSerializedField().getModifiedCounter();
    try {
  ...
  /**
   * refreshMcrLineCharge is used to refresh the latest value of a variable from the Serialized
