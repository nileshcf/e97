...
/**
 * The class GenTransSpiTableGroupSerialized is used to define offsets in order to serialize in a
 * fixed String
 *
...
public class GenTransSpiTableGroupSerialized extends Field {
  ...
  public boolean isGenTransSpiTableModified() {
    ...
    localGenTransSpiTableCounter = sharedCounter;
    return hasModified;
  }
  ...
  protected void serializeGenTransSpiTable(char[] genTransSpiTable) {
    ...
    arraycopy(
        genTransSpiTable, 0, getStringValue(), beginGenTransSpiTable, GEN_TRANS_SPI_TABLE_LEN);
  ...
  /**
   * refreshGenTransSpiTable is used to refresh the latest value of a variable from the Serialized
   * String the most common reason to serialize an Object as a string in to write to a file. There
   * can be several other reasons for serialization as well
   */
  public char[] refreshGenTransSpiTable() {
    return (substring(
        getStringValue(), beginGenTransSpiTable, beginGenTransSpiTable + GEN_TRANS_SPI_TABLE_LEN));
  }
}
