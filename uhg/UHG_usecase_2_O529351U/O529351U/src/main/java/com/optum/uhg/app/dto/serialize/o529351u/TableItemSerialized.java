...
/**
 * The class TableItemSerialized is used to define offsets in order to serialize in a fixed String
 *
...
public class TableItemSerialized extends Field {
  ...
  /**
   * sets parent for this TableItemSerialized to the parent and set the serialize offset to
   * parameter begin
   *
   * @param parent
  ...
  public boolean isGenericTranslModified() {
    ...
    localGenericTranslCounter = sharedCounter;
    return hasModified;
  }
  ...
  protected long serializeGenericTransl(char[] value) {
    ...
    replaceValue(padNumber(6, value, false /*isSigned?*/), beginGenericTransl, 6);
    localGenericTranslCounter = shareString.getSerializedField().getModifiedCounter();
    return genericTransl;
  }
  ...
  /**
   * refreshGenericTransl is used to refresh the latest value of a variable from the Serialized
