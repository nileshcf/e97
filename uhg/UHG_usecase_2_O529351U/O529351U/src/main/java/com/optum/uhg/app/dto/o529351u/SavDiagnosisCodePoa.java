...
public class SavDiagnosisCodePoa extends SavDiagnosisCodePoaSerialized {
  ...
  /**
   * Returns the value of savDiagnosisCode Corresponding COBOL Variable is SAV-DIAGNOSIS-CODE
   *
  ...
  public char[] getSavDiagnosisCode(int index) {
    if (index < 0) {
      ...
      logger.trace(
          "savDiagnosisCode - Array index exceeded max Size {}, resetting it to max allowed",
          SAV_DIAGNOSIS_CODE_SIZE);
    }
    return getCharArray(
        (beginSavDiagnosisCode + index * SAV_DIAGNOSIS_CODE_LEN), SAV_DIAGNOSIS_CODE_LEN);
  }
  ...
  /**
   * Update SavDiagnosisCode with the passed value at a given index Corresponding COBOL Variable is
   * SAV-DIAGNOSIS-CODE
   *
   * @param index
   * @param value
  ...
  /**
   * Update SavDiagnosisCode with the passed value at a given index and serializes if setModified is
   * true
   *
   * @param index
   * @param value
  ...
  private void setSavDiagnosisCode(int index, char[] value, boolean setModified) {
    if (index < 0) {
      logger.trace(
          "Array index was {} for setSavDiagnosisCode(int,String,boolean), reset it to 0", index);
      index = 0;
    }
