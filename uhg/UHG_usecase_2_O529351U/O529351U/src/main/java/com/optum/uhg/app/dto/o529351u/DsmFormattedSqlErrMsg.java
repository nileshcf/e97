...
public class DsmFormattedSqlErrMsg extends DsmFormattedSqlErrMsgSerialized {
  ...
  /**
   * Returns the value of dsmFrmtdSqlErrMsg Corresponding COBOL Variable is DSM-FRMTD-SQL-ERR-MSG
   *
  ...
  public char[] getDsmFrmtdSqlErrMsg(int index) {
    if (index < 0) {
      ...
      logger.trace(
          "dsmFrmtdSqlErrMsg - Array index exceeded max Size {}, resetting it to max allowed",
          DSM_FRMTD_SQL_ERR_MSG_SIZE);
    }
    return getCharArray(
        (beginDsmFrmtdSqlErrMsg + index * DSM_FRMTD_SQL_ERR_MSG_LEN), DSM_FRMTD_SQL_ERR_MSG_LEN);
  }
  ...
  /**
   * Update DsmFrmtdSqlErrMsg with the passed value at a given index Corresponding COBOL Variable is
   * DSM-FRMTD-SQL-ERR-MSG
   *
   * @param index
   * @param value
   */
  ...
  /**
   * Update DsmFrmtdSqlErrMsg with the passed value at a given index and serializes if setModified
   * is true
   *
   * @param index
   * @param value
  ...
  private void setDsmFrmtdSqlErrMsg(int index, char[] value, boolean setModified) {
    if (index < 0) {
      logger.trace(
          "Array index was {} for setDsmFrmtdSqlErrMsg(int,String,boolean), reset it to 0", index);
      index = 0;
    }
  ...
  /**
   * initializes DsmFormattedSqlErrMsg String fields defaults to Spaces short/int/long defaults to
