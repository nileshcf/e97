...
/**
 * The Mcsearch application can be started in three different ways
 * 1. Via mainframe JCL
	...
	public static void main(String[] args) {
		...
		if (StringUtils.hasText(appType) && WebApplicationType.NONE.equals(WebApplicationType.valueOf(appType.trim().toUpperCase())))
            ...
            GlobalExecutorCtx.load(CFApplication.class, context);
		 		...
		 		Mcsearch mcsearch = (Mcsearch) context.getBean("mcsearch");
        	    mcsearch.process(new GlobalExecutorCtx().getContext(McsearchCtx.class)); /*  Mcsearch requires no input parameters  */
