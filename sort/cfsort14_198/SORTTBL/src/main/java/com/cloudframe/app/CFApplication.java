...
/**
 * The Sorttbl application can be started in three different ways
 * 1. Via mainframe JCL
	...
	public static void main(String[] args) {
		...
		if (StringUtils.hasText(appType) && WebApplicationType.NONE.equals(WebApplicationType.valueOf(appType.trim().toUpperCase())))
            ...
            GlobalExecutorCtx.load(CFApplication.class, context);
		 		...
		 		Sorttbl sorttbl = (Sorttbl) context.getBean("sorttbl");
        	    sorttbl.process(new GlobalExecutorCtx().getContext(SorttblCtx.class)); /*  Sorttbl requires no input parameters  */
