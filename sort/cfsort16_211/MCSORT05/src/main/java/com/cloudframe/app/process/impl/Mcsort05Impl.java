...
*/
  ...
  import com.cloudframe.app.mcsort05.Mcsort05Ctx;
  import com.cloudframe.app.process.Mcsort05;
  ...
  public class Mcsort05Impl extends CommonProcess implements Mcsort05, InitializingBean, DisposableBean {
  
  Logger logger = LoggerFactory.getLogger(Mcsort05Impl.class);
...
//  PERFORM 2000-SORT-FILE
          sortFile(programCtx.getSortFileInCtx());/*2000-SORT-FILE*/
          if (programCtx.isProgramEnded()) {
              return ;
          }
//  PERFORM 9500-PROCESS-TERMINATION
          processTermination(programCtx.getProcessTerminationInCtx());/*9500-PROCESS-TERMINATION*/
          if (programCtx.isProgramEnded()) {
              return ;
          }
//  GOBACK
...
Mcsort05Ctx programCtx = methodIn.getMcsort05Ctx();
...
//  MOVE 300-PGM-MCSORT05 TO IP996011-PGM-ID
          methodOut.setIp996011PgmId(methodOut.getPgmMcsort05300());
...
//  MOVE WHEN-COMPILED TO IP996011-WHEN-COMPILED
          methodOut.setIp996011WhenCompiled(CONSTANTS.LITERAL_601786682);
...
//  DISPLAY 301-END-OF-STEP-MSG
          logger.info(new String(methodIn.getEndOfStepMsg301())); 
