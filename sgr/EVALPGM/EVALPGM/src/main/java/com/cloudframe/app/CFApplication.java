...
/**
 * The Evalpgm application can be started in three different ways
 * 1. Via mainframe JCL
	...
	public static void main(String[] args) {
		...
		if (StringUtils.hasText(appType) && WebApplicationType.NONE.equals(WebApplicationType.valueOf(appType.trim().toUpperCase())))
            ...
            GlobalExecutorCtx.load(CFApplication.class, context);
		 		...
		 		Evalpgm evalpgm = (Evalpgm) context.getBean("evalpgm");
        	    evalpgm.process(new GlobalExecutorCtx().getContext(EvalpgmCtx.class)); /*  Evalpgm requires no input parameters  */
