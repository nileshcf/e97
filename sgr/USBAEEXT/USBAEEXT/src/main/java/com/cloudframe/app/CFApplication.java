...
/**
 * The Usbaeext application can be started in three different ways
 * 1. Via mainframe JCL
	...
	public static void main(String[] args) {
		...
		if (StringUtils.hasText(appType) && WebApplicationType.NONE.equals(WebApplicationType.valueOf(appType.trim().toUpperCase())))
            ...
            GlobalExecutorCtx.load(CFApplication.class, context);
		 		...
		 		Usbaeext usbaeext = (Usbaeext) context.getBean("usbaeext");
        	    usbaeext.process(new GlobalExecutorCtx().getContext(UsbaeextCtx.class)); /*  Usbaeext requires no input parameters  */
