...
/**
 * The Addrchk application can be started in three different ways
 * 1. Via mainframe JCL
	...
	public static void main(String[] args) {
		...
		if (StringUtils.hasText(appType) && WebApplicationType.NONE.equals(WebApplicationType.valueOf(appType.trim().toUpperCase())))
            ...
            GlobalExecutorCtx.load(CFApplication.class, context);
		 		...
		 		Addrchk addrchk = (Addrchk) context.getBean("addrchk");
        	    addrchk.process(new GlobalExecutorCtx().getContext(AddrchkCtx.class)); /*  Addrchk requires no input parameters  */
