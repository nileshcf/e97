...
/**
 * The Dbissues application can be started in three different ways
 * 1. Via mainframe JCL
	...
	public static void main(String[] args) {
		...
		if (StringUtils.hasText(appType) && WebApplicationType.NONE.equals(WebApplicationType.valueOf(appType.trim().toUpperCase())))
            ...
            GlobalExecutorCtx.load(CFApplication.class, context);
		 		...
		 		Dbissues dbissues = (Dbissues) context.getBean("dbissues");
        	    dbissues.process(new GlobalExecutorCtx().getContext(DbissuesCtx.class)); /*  Dbissues requires no input parameters  */
