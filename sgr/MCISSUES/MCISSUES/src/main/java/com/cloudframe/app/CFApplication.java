...
/**
 * The Mcissues application can be started in three different ways
 * 1. Via mainframe JCL
	...
	public static void main(String[] args) {
		...
		if (StringUtils.hasText(appType) && WebApplicationType.NONE.equals(WebApplicationType.valueOf(appType.trim().toUpperCase())))
            ...
            GlobalExecutorCtx.load(CFApplication.class, context);
		 		...
		 		Mcissues mcissues = (Mcissues) context.getBean("mcissues");
        	    mcissues.process(new GlobalExecutorCtx().getContext(McissuesCtx.class)); /*  Mcissues requires no input parameters  */
