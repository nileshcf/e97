...
*/
  ...
  import com.cloudframe.app.unstrng0.Unstrng0Ctx.*;
  import com.cloudframe.app.unstrng0.Unstrng0Ctx;
  import com.cloudframe.app.process.Unstrng0;
  ...
  public class Unstrng0Impl extends CommonProcess implements Unstrng0 {
  
  Logger logger = LoggerFactory.getLogger(Unstrng0Impl.class);
  ...
  private Map<String, Pattern> patternMap = new HashMap<>();
      ...
      * @throws CFException
      */
      public int process(Unstrng0Ctx programCtx) throws Exception {
...
ProcessOutCtx methodOut = methodIn.getProcessOutCtx();
          ...
          methodOut.setRetain(CONSTANTS.LITERAL_301812617);
//  UNSTRING INPUT-REC DELIMITED BY '|' INTO WS-VAL1 , WS-VAL2 , WS-VAL3 , WS-VAL4 , WS-VAL5 , WS-VAL6 , WS-VAL7 , WS-VAL8 , WS-VAL9 , WS-VALA WS-VALB COUNT IN WS-COUNT END-UNSTRING
          ...
          actualUnstringStr = methodIn.getInputRec();
          ...
          try{
          	...
          	methodOut.setVal1(resultKey);
          	
          	loop++;
          	resultKey = (char[])result.get(loop).getKey();
          	resultValue = (char[])result.get(loop).getValue();
          	methodOut.setVal2(resultKey);
          	
          	loop++;
          	resultKey = (char[])result.get(loop).getKey();
          	resultValue = (char[])result.get(loop).getValue();
          	methodOut.setVal3(resultKey);
          	
          	loop++;
          	resultKey = (char[])result.get(loop).getKey();
          	resultValue = (char[])result.get(loop).getValue();
          	methodOut.setVal4(resultKey);
          	
          	loop++;
          	resultKey = (char[])result.get(loop).getKey();
          	resultValue = (char[])result.get(loop).getValue();
          	methodOut.setVal5(resultKey);
          	
          	loop++;
          	resultKey = (char[])result.get(loop).getKey();
          	resultValue = (char[])result.get(loop).getValue();
          	methodOut.setVal6(resultKey);
          	
          	loop++;
          	resultKey = (char[])result.get(loop).getKey();
          	resultValue = (char[])result.get(loop).getValue();
          	methodOut.setVal7(resultKey);
          	
          	loop++;
          	resultKey = (char[])result.get(loop).getKey();
          	resultValue = (char[])result.get(loop).getValue();
          	methodOut.setVal8(resultKey);
          	
          	loop++;
          	resultKey = (char[])result.get(loop).getKey();
          	resultValue = (char[])result.get(loop).getValue();
          	methodOut.setVal9(resultKey);
          	
          	loop++;
          	resultKey = (char[])result.get(loop).getKey();
          	resultValue = (char[])result.get(loop).getValue();
          	methodOut.setVala(resultKey);
          	
          	loop++;
          	resultKey = (char[])result.get(loop).getKey();
          	resultValue = (char[])result.get(loop).getValue();
          	methodOut.setValb(resultKey);
          methodOut.setCount(resultKey.length);
...
//  DISPLAY '1. WS-COUNT ' WS-COUNT
          logger.info("1. WS-COUNT {}", String.valueOf(methodOut.getCount())); 
//  DISPLAY '2. WS-VALB  ' WS-VALB
          logger.info("2. WS-VALB  {}", new String(methodOut.getValb())); 
//  DISPLAY '3. WS-VALC  ' WS-VALC
          logger.info("3. WS-VALC  {}", new String(methodIn.getValc())); 
//  DISPLAY '3A. RETAIN ' WS-RETAIN (1 : 50)
          logger.info("3A. RETAIN {}", new String(substring(methodOut.getRetain(),0,50))); 
...
//  UNSTRING INPUT-REC-OVERFLW1 DELIMITED BY '|' INTO WS-VAL1 , WS-VAL2 , WS-VAL3 , WS-VAL4 , WS-VAL5 , WS-VAL6 , WS-VAL7 , WS-VAL8 , WS-VAL9 , WS-VALA WS-VALB COUNT IN WS-COUNT END-UNSTRING
          //This will have the no of characters parsed while un string. On NoOverflow pointerCnt == actualstring.length
          actualUnstringStr = methodIn.getInputRecOverflw1();
          //Construct an ArrayList by the given Delimited String. '+' will be added when there is ALL.EG ALL $, then delimited String will be $+
          splitBy = getArrayList(CONSTANTS.LITERAL_36552);
          //Create a RegularExpression String from the above delimited list (updateRegEx will add escape character \\ if in need, also group the values when required)
          updatedRegEx = updateRegEx(splitBy);
...
//  DISPLAY '4. WS-COUNT ' WS-COUNT
          logger.info("4. WS-COUNT {}", String.valueOf(methodOut.getCount())); 
//  DISPLAY '5. WS-VALB  ' WS-VALB
          logger.info("5. WS-VALB  {}", new String(methodOut.getValb())); 
//  DISPLAY '6. WS-VALC  ' WS-VALC
          logger.info("6. WS-VALC  {}", new String(methodIn.getValc())); 
//  DISPLAY '6A. RETAIN ' WS-RETAIN (1 : 50)
          logger.info("6A. RETAIN {}", new String(substring(methodOut.getRetain(),0,50))); 
...
//  UNSTRING INPUT-REC-OVERFLW2 DELIMITED BY '|' INTO WS-VAL1 , WS-VAL2 , WS-VAL3 , WS-VAL4 , WS-VAL5 , WS-VAL6 , WS-VAL7 , WS-VAL8 , WS-VAL9 , WS-VALA WS-VALB COUNT IN WS-COUNT END-UNSTRING
          //This will have the no of characters parsed while un string. On NoOverflow pointerCnt == actualstring.length
          actualUnstringStr = methodIn.getInputRecOverflw2();
          //Construct an ArrayList by the given Delimited String. '+' will be added when there is ALL.EG ALL $, then delimited String will be $+
          splitBy = getArrayList(CONSTANTS.LITERAL_36552);
          //Create a RegularExpression String from the above delimited list (updateRegEx will add escape character \\ if in need, also group the values when required)
          updatedRegEx = updateRegEx(splitBy);
...
//  DISPLAY '7. WS-COUNT ' WS-COUNT
          logger.info("7. WS-COUNT {}", String.valueOf(methodOut.getCount())); 
//  DISPLAY '8. WS-VALB  ' WS-VALB
          logger.info("8. WS-VALB  {}", new String(methodOut.getValb())); 
//  DISPLAY '9. WS-VALC  ' WS-VALC
          logger.info("9. WS-VALC  {}", new String(methodIn.getValc())); 
//  DISPLAY '9A. RETAIN ' WS-RETAIN (1 : 50)
          logger.info("9A. RETAIN {}", new String(substring(methodOut.getRetain(),0,50))); 
...
//  UNSTRING INPUT-REC-OVERFLW3 DELIMITED BY '|' INTO WS-VAL1 , WS-VAL2 , WS-VAL3 , WS-VAL4 , WS-VAL5 , WS-VAL6 , WS-VAL7 , WS-VAL8 , WS-VAL9 , WS-VALA WS-VALB COUNT IN WS-COUNT END-UNSTRING
          //This will have the no of characters parsed while un string. On NoOverflow pointerCnt == actualstring.length
          actualUnstringStr = methodIn.getInputRecOverflw3();
          //Construct an ArrayList by the given Delimited String. '+' will be added when there is ALL.EG ALL $, then delimited String will be $+
          splitBy = getArrayList(CONSTANTS.LITERAL_36552);
          //Create a RegularExpression String from the above delimited list (updateRegEx will add escape character \\ if in need, also group the values when required)
          updatedRegEx = updateRegEx(splitBy);
...
//  DISPLAY '10. WS-COUNT ' WS-COUNT
          logger.info("10. WS-COUNT {}", String.valueOf(methodOut.getCount())); 
//  DISPLAY '11. WS-VALB  ' WS-VALB
          logger.info("11. WS-VALB  {}", new String(methodOut.getValb())); 
//  DISPLAY '12. WS-VALC  ' WS-VALC
          logger.info("12. WS-VALC  {}", new String(methodIn.getValc())); 
//  DISPLAY '12A. RETAIN ' WS-RETAIN (1 : 50)
          logger.info("12A. RETAIN {}", new String(substring(methodOut.getRetain(),0,50))); 

// *
          // MOVE 4 TO WS-POINTER
          methodOut.setPointer((long)4);
//  UNSTRING WS-DATA0 (1 : WS-DATA0-LEN ) DELIMITED BY ',' INTO WS-FIELD1 WS-FIELD2 WS-FIELD3 WS-FIELD4 WS-FIELD5
          //This will have the no of characters parsed while un string. On NoOverflow pointerCnt == actualstring.length
          actualUnstringStr = substring(methodIn.getData0(),0,methodIn.getData0Len());
          //Construct an ArrayList by the given Delimited String. '+' will be added when there is ALL.EG ALL $, then delimited String will be $+
          ...
          try{
          	resultKey = (char[])result.get(loop).getKey();
          	resultValue = (char[])result.get(loop).getValue();
          	methodOut.setField1(resultKey);
          	
          	loop++;
          	resultKey = (char[])result.get(loop).getKey();
          	resultValue = (char[])result.get(loop).getValue();
          	methodOut.setField2(resultKey);
          	
          	loop++;
          	resultKey = (char[])result.get(loop).getKey();
          	resultValue = (char[])result.get(loop).getValue();
          	methodOut.setField3(resultKey);
          	
          	loop++;
          	resultKey = (char[])result.get(loop).getKey();
          	resultValue = (char[])result.get(loop).getValue();
          	methodOut.setField4(resultKey);
          	
          	loop++;
          	resultKey = (char[])result.get(loop).getKey();
          	resultValue = (char[])result.get(loop).getValue();
          	methodOut.setField5(resultKey);
...
//  DISPLAY ' FIELD 1 ' WS-FIELD1
          logger.info(" FIELD 1 {}", new String(methodOut.getField1())); 
//  DISPLAY ' FIELD 2 ' WS-FIELD2
          logger.info(" FIELD 2 {}", new String(methodOut.getField2())); 
//  DISPLAY ' FIELD 3 ' WS-FIELD3
          logger.info(" FIELD 3 {}", new String(methodOut.getField3())); 
//  DISPLAY ' FIELD 4 ' WS-FIELD4
          logger.info(" FIELD 4 {}", new String(methodOut.getField4())); 
//  DISPLAY ' FIELD 5 ' WS-FIELD5
          logger.info(" FIELD 5 {}", new String(methodOut.getField5())); 
//  DISPLAY ' ---------------------------------  '
          logger.info(" ---------------------------------  "); 
//  UNSTRING WS-DATA DELIMITED BY '%' OR ALL '$' INTO WS-FIELD1 DELIMITER IN WS-DELIM1 COUNT IN WS-COUNT1 WS-FIELD2 COUNT IN WS-COUNT2 WS-FIELD3 DELIMITER IN WS-DELIM3 COUNT IN WS-COUNT3 WS-FIELD4 DELIMITER IN WS-DELIM4 COUNT IN WS-COUNT4 WS-FIELD5 COUNT IN WS-COUNT5 POINTER WS-POINTER TALLYING WS-TALLY END-UNSTRING
          pointerCnt = 0;
          actualUnstringStr =  Arrays.copyOfRange(methodIn.getData(),getIndex((int) methodOut.getPointer()), methodIn.getData().length);
          //methodOut.setPointer(methodIn.getData().length()+1);
          //Construct an ArrayList by the given Delimited String. '+' will be added when there is ALL.EG ALL $, then delimited String will be $+
          ...
          try{
          	...
          	resultValue = (char[])result.get(loop).getValue();
          	methodOut.setField1(resultKey);
          methodIn.setDelim1(removeDuplicateChars(resultValue));
          methodOut.setCount1(resultKey.length);
          	pointerCnt += resultKey.length + resultValue.length;
          	loop++;
          	resultKey = (char[])result.get(loop).getKey();
          	resultValue = (char[])result.get(loop).getValue();
          	methodOut.setField2(resultKey);
          methodOut.setCount2(resultKey.length);
          	pointerCnt += resultKey.length + resultValue.length;
          	loop++;
          	resultKey = (char[])result.get(loop).getKey();
          	resultValue = (char[])result.get(loop).getValue();
          	methodOut.setField3(resultKey);
          methodIn.setDelim3(removeDuplicateChars(resultValue));
          methodOut.setCount3(resultKey.length);
          	pointerCnt += resultKey.length + resultValue.length;
          	loop++;
          	resultKey = (char[])result.get(loop).getKey();
          	resultValue = (char[])result.get(loop).getValue();
          	methodOut.setField4(resultKey);
          methodIn.setDelim4(removeDuplicateChars(resultValue));
          methodOut.setCount4(resultKey.length);
          	pointerCnt += resultKey.length + resultValue.length;
          	loop++;
          	resultKey = (char[])result.get(loop).getKey();
          	resultValue = (char[])result.get(loop).getValue();
          	methodOut.setField5(resultKey);
          methodOut.setCount5(resultKey.length);
          	pointerCnt += resultKey.length + resultValue.length;
          	loop++;
          }
          catch(IndexOutOfBoundsException e ){
          ...
          methodOut.setPointer((int) methodOut.getPointer()+pointerCnt);
          	methodIn.setTally(loop);	
//  DISPLAY ' FIELD 1 ' WS-FIELD1 ' DELIM ' WS-DELIM1 ' COUNT ' WS-COUNT1
          logger.info(" FIELD 1 {} DELIM {} COUNT {}", new String(methodOut.getField1()), new String(methodIn.getDelim1()), String.valueOf(methodOut.getCount1())); 
//  DISPLAY ' FIELD 2 ' WS-FIELD2 ' COUNT ' WS-COUNT2
          logger.info(" FIELD 2 {} COUNT {}", new String(methodOut.getField2()), String.valueOf(methodOut.getCount2())); 
//  DISPLAY ' FIELD 3 ' WS-FIELD3 ' DELIM ' WS-DELIM3 ' COUNT ' WS-COUNT3
          logger.info(" FIELD 3 {} DELIM {} COUNT {}", new String(methodOut.getField3()), new String(methodIn.getDelim3()), String.valueOf(methodOut.getCount3())); 
//  DISPLAY ' FIELD 4 ' WS-FIELD4 ' DELIM ' WS-DELIM4 ' COUNT ' WS-COUNT4
          logger.info(" FIELD 4 {} DELIM {} COUNT {}", new String(methodOut.getField4()), new String(methodIn.getDelim4()), String.valueOf(methodOut.getCount4())); 
//  DISPLAY ' FIELD 5 ' WS-FIELD5 ' COUNT ' WS-COUNT5
          logger.info(" FIELD 5 {} COUNT {}", new String(methodOut.getField5()), String.valueOf(methodOut.getCount5())); 
//  DISPLAY ' TALLY ' WS-TALLY
          logger.info(" TALLY {}", String.valueOf(methodIn.getTally())); 
//  GOBACK
