...
/**
 * The Varlen application can be started in three different ways
 * 1. Via mainframe JCL
	...
	public static void main(String[] args) {
		...
		if (StringUtils.hasText(appType) && WebApplicationType.NONE.equals(WebApplicationType.valueOf(appType.trim().toUpperCase())))
            ...
            GlobalExecutorCtx.load(CFApplication.class, context);
		 		...
		 		Varlen varlen = (Varlen) context.getBean("varlen");
        	    varlen.process(new GlobalExecutorCtx().getContext(VarlenCtx.class)); /*  Varlen requires no input parameters  */
