...
/**
 * The Echoqsam application can be started in three different ways
 * 1. Via mainframe JCL
	...
	public static void main(String[] args) {
		...
		if (StringUtils.hasText(appType) && WebApplicationType.NONE.equals(WebApplicationType.valueOf(appType.trim().toUpperCase())))
            ...
            GlobalExecutorCtx.load(CFApplication.class, context);
		 		...
		 		Echoqsam echoqsam = (Echoqsam) context.getBean("echoqsam");
        	    echoqsam.process(new GlobalExecutorCtx().getContext(EchoqsamCtx.class)); /*  Echoqsam requires no input parameters  */
