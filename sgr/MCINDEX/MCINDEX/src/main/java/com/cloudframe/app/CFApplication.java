...
/**
 * The Mcindex application can be started in three different ways
 * 1. Via mainframe JCL
	...
	public static void main(String[] args) {
		...
		if (StringUtils.hasText(appType) && WebApplicationType.NONE.equals(WebApplicationType.valueOf(appType.trim().toUpperCase())))
            ...
            GlobalExecutorCtx.load(CFApplication.class, context);
		 		...
		 		Mcindex mcindex = (Mcindex) context.getBean("mcindex");
        	    mcindex.process(new GlobalExecutorCtx().getContext(McindexCtx.class)); /*  Mcindex requires no input parameters  */
