...
/**
 * The Asaprnt application can be started in three different ways
 * 1. Via mainframe JCL
	...
	public static void main(String[] args) {
		...
		if (StringUtils.hasText(appType) && WebApplicationType.NONE.equals(WebApplicationType.valueOf(appType.trim().toUpperCase())))
            ...
            GlobalExecutorCtx.load(CFApplication.class, context);
		 		...
		 		Asaprnt asaprnt = (Asaprnt) context.getBean("asaprnt");
        	    asaprnt.process(new GlobalExecutorCtx().getContext(AsaprntCtx.class)); /*  Asaprnt requires no input parameters  */
