...
/**
 * The Blkdblk application can be started in three different ways
 * 1. Via mainframe JCL
	...
	public static void main(String[] args) {
		...
		if (StringUtils.hasText(appType) && WebApplicationType.NONE.equals(WebApplicationType.valueOf(appType.trim().toUpperCase())))
            ...
            GlobalExecutorCtx.load(CFApplication.class, context);
		 		...
		 		Blkdblk blkdblk = (Blkdblk) context.getBean("blkdblk");
        	    blkdblk.process(new GlobalExecutorCtx().getContext(BlkdblkCtx.class)); /*  Blkdblk requires no input parameters  */
