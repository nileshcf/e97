...
public class CacheKeyFixedAreaSerialized extends Field {
  ...
  protected int beginCacheKeyOverflowInd01;
  ...
  protected void init(int begin) {
    ...
    beginCacheKeyOverflowInd01 = getStartOffset() + 12; // set offset for serialization
  ...
  int localCacheKeyOverflowInd01Counter = -1;

  public boolean isCacheKeyOverflowInd01Modified() {
    int sharedCounter = shareString.getSerializedField().getModifiedCounter();
    boolean hasModified = localCacheKeyOverflowInd01Counter != sharedCounter;
    localCacheKeyOverflowInd01Counter = sharedCounter;
    return hasModified;
  }

  protected static final int CACHE_KEY_OVERFLOW_IND_01_LEN = 1;
  /** serialize this CacheKeyOverflowInd01 */
  protected void serializeCacheKeyOverflowInd01(char[] cacheKeyOverflowInd01) {
    shareString.getSerializedField().incrementCounter();
    arraycopy(
        cacheKeyOverflowInd01,
        0,
        getStringValue(),
        beginCacheKeyOverflowInd01,
        CACHE_KEY_OVERFLOW_IND_01_LEN);
    localCacheKeyOverflowInd01Counter = shareString.getSerializedField().getModifiedCounter();
  }

  protected char[] checkCacheKeyOverflowInd01Constraints(char[] value) {
    return super.checkConstraints(value, 1, false, false);
  }
  /**
   * refreshCacheKeyOverflowInd01 is used to refresh the latest value of a variable from the
   ...
   */
  public char[] refreshCacheKeyOverflowInd01() {
    return (substring(
        getStringValue(),
        beginCacheKeyOverflowInd01,
        beginCacheKeyOverflowInd01 + CACHE_KEY_OVERFLOW_IND_01_LEN));
  }
}
