...
public class BhiReturnArea extends BhiReturnAreaSerialized {
  ...
  /**
   * initializes BhiReturnArea String fields defaults to Spaces short/int/long defaults to zero
   * BigDecimal defaults to BigDecimal.ZERO
   */
  public void initialize() {
    setBhiDb2Table(CONSTANTS.SPACE_30);
    setBhiDb2ParaNm(CONSTANTS.SPACE_30);
    setBhiDb2Action(CONSTANTS.SPACE_8);
    setBhiDb2RetCode(CFUtil.cobolNumberFormatter(CONSTANTS.FORMAT261490060, "0".toCharArray()));
    setBhiRetReasonCode(CONSTANTS.SPACE_2);
    setBhiNumOccurs((short) 0);
    if (bhiReturnRow.isEmpty()) {
      // fill the Array to the full capacity
      for (int index = 0; index < BHI_RETURN_ROW_SIZE; index++) {
        BhiReturnRow newElement =
            new BhiReturnRow(
                this, beginBhiReturnRow + index * BhiReturnRow.getBhiReturnRowFieldLength());
        newElement.initialize();
        bhiReturnRow.add(newElement);
      }
    } else {
      if (bhiReturnRow.size() < BHI_RETURN_ROW_SIZE) {
        // prefill it first
        for (int index = bhiReturnRow.size(); index < BHI_RETURN_ROW_SIZE; index++) {
          BhiReturnRow newElement =
              new BhiReturnRow(
                  this, beginBhiReturnRow + index * BhiReturnRow.getBhiReturnRowFieldLength());
          bhiReturnRow.add(newElement);
      ...
      for (int index = 0; index < BHI_RETURN_ROW_SIZE; index++) {
        BhiReturnRow bhiReturnRowVar = bhiReturnRow.get(index);
        if (bhiReturnRowVar == null) {
          bhiReturnRowVar =
              new BhiReturnRow(
                  this, beginBhiReturnRow + index * BhiReturnRow.getBhiReturnRowFieldLength());
          bhiReturnRow.set(index, bhiReturnRowVar);
        }
        bhiReturnRowVar.initialize();
      }
    }
  }
