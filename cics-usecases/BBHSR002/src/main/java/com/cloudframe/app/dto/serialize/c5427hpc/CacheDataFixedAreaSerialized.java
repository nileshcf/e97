...
public class CacheDataFixedAreaSerialized extends Field {
  ...
  protected int beginCacheDataOverflowInd;
  ...
  protected void init(int begin) {
    ...
    beginCacheDataOverflowInd = getStartOffset() + 8; // set offset for serialization
  ...
  protected static final int CACHE_DATA_OVERFLOW_IND_LEN = 1;
  /** serialize this CacheDataOverflowInd */
  protected void serializeCacheDataOverflowInd(char[] cacheDataOverflowInd) {
    shareString.getSerializedField().incrementCounter();
    arraycopy(
        cacheDataOverflowInd,
        0,
        getStringValue(),
        beginCacheDataOverflowInd,
        CACHE_DATA_OVERFLOW_IND_LEN);
    localCacheDataOverflowIndCounter = shareString.getSerializedField().getModifiedCounter();
  }
  ...
  /**
   * refreshCacheDataOverflowInd is used to refresh the latest value of a variable from the
   ...
   */
  public char[] refreshCacheDataOverflowInd() {
    return (substring(
        getStringValue(),
        beginCacheDataOverflowInd,
        beginCacheDataOverflowInd + CACHE_DATA_OVERFLOW_IND_LEN));
  }
}
