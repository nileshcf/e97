...
/**
 * The Edjaddrs application can be started in three different ways
 * 1. Via mainframe JCL
	...
	public static void main(String[] args) {
		...
		if (StringUtils.hasText(appType) && WebApplicationType.NONE.equals(WebApplicationType.valueOf(appType.trim().toUpperCase())))
            ...
            GlobalExecutorCtx.load(CFApplication.class, context);
		 		...
		 		Edjaddrs edjaddrs = (Edjaddrs) context.getBean("edjaddrs");
        	    edjaddrs.process(new GlobalExecutorCtx().getContext(EdjaddrsCtx.class)); /*  Edjaddrs requires no input parameters  */
