...
/**
 * The Mdsnum application can be started in three different ways
 * 1. Via mainframe JCL
	...
	public static void main(String[] args) {
		...
		if (StringUtils.hasText(appType) && WebApplicationType.NONE.equals(WebApplicationType.valueOf(appType.trim().toUpperCase())))
            ...
            GlobalExecutorCtx.load(CFApplication.class, context);
		 		...
		 		Mdsnum mdsnum = (Mdsnum) context.getBean("mdsnum");
        	    mdsnum.process(new GlobalExecutorCtx().getContext(MdsnumCtx.class)); /*  Mdsnum requires no input parameters  */
