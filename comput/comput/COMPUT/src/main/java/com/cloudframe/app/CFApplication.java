...
/**
 * The Comput application can be started in three different ways
 * 1. Via mainframe JCL
	...
	public static void main(String[] args) {
		...
		if (StringUtils.hasText(appType) && WebApplicationType.NONE.equals(WebApplicationType.valueOf(appType.trim().toUpperCase())))
            ...
            GlobalExecutorCtx.load(CFApplication.class, context);
		 		...
		 		Comput comput = (Comput) context.getBean("comput");
        	    comput.process(new GlobalExecutorCtx().getContext(ComputCtx.class)); /*  Comput requires no input parameters  */
